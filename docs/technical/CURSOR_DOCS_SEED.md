# Cursor Docs Seed (RenoveJá)

Use these links in Cursor **Docs > Add Doc** (Settings > Features) ou via **@Docs > Add New Doc** no chat.

## cursorDocs (lista completa para adicionar)
- https://docs.expo.dev/
- https://docs.expo.dev/router/introduction/
- https://reactnative.dev/docs/getting-started
- https://tanstack.com/query/latest/docs/framework/react/overview
- https://learn.microsoft.com/aspnet/core/?view=aspnetcore-8.0
- https://learn.microsoft.com/aspnet/core/fundamentals/best-practices?view=aspnetcore-8.0
- https://learn.microsoft.com/dotnet/architecture/modern-web-apps-azure/
- https://learn.microsoft.com/dotnet/core/testing/
- https://learn.microsoft.com/ef/core/
- https://refactoring.guru/design-patterns/csharp
- https://deviq.com/principles/solid
- https://owasp.org/www-project-top-ten/
- https://cheatsheetseries.owasp.org/

## 1) Base do projeto (obrigatório)
- https://docs.expo.dev/
- https://docs.expo.dev/router/introduction/
- https://reactnative.dev/docs/getting-started
- https://tanstack.com/query/latest/docs/framework/react/overview
- https://learn.microsoft.com/aspnet/core/?view=aspnetcore-8.0
- https://learn.microsoft.com/dotnet/architecture/modern-web-apps-azure/

## 2) .NET boas práticas / arquitetura / SOLID
- https://learn.microsoft.com/dotnet/standard/design-guidelines/
- https://learn.microsoft.com/dotnet/architecture/microservices/
- https://learn.microsoft.com/aspnet/core/fundamentals/best-practices?view=aspnetcore-8.0
- https://learn.microsoft.com/ef/core/
- https://refactoring.guru/design-patterns
- https://refactoring.guru/design-patterns/csharp
- https://deviq.com/principles/solid
- https://martinfowler.com/bliki/

## 3) Qualidade, testes e segurança
- https://learn.microsoft.com/dotnet/core/testing/
- https://learn.microsoft.com/aspnet/core/test/?view=aspnetcore-8.0
- https://owasp.org/www-project-top-ten/
- https://cheatsheetseries.owasp.org/

## 4) Frontend UX, acessibilidade e performance
- https://www.w3.org/WAI/fundamentals/accessibility-intro/
- https://web.dev/performance
- https://react.dev/learn

## 5) DevOps / versionamento / entrega
- https://learn.microsoft.com/azure/devops/repos/git/gitworkflow
- https://www.conventionalcommits.org/en/v1.0.0/
- https://semver.org/

## Ordem recomendada para adicionar primeiro (Top 10)
1. https://docs.expo.dev/
2. https://reactnative.dev/docs/getting-started
4. https://learn.microsoft.com/aspnet/core/?view=aspnetcore-8.0
5. https://learn.microsoft.com/aspnet/core/fundamentals/best-practices?view=aspnetcore-8.0
6. https://learn.microsoft.com/dotnet/core/testing/
7. https://learn.microsoft.com/dotnet/architecture/modern-web-apps-azure/
8. https://refactoring.guru/design-patterns/csharp
9. https://deviq.com/principles/solid
10. https://owasp.org/www-project-top-ten/
