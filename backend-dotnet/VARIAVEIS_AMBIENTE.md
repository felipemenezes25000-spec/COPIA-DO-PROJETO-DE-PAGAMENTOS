...
| Causa | Verificação | Solução |
|-------|-------------|---------|
| `OpenAI:ApiKey` ausente | Log: `[Whisper] OpenAI:ApiKey não configurada` | Definir `OpenAI__ApiKey` ou `OpenAI:ApiKey` em appsettings (transcrição usa mesma chave que GPT-4o) |
