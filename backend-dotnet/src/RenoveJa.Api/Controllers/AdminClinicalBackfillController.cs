...
{
    // Single-writer lock: prevents two admins from running the backfill concurrently,
    // which would otherwise race on MedicalDocument existence checks and potentially duplicate rows.
    ...
    /// Idempotente: não duplica se já existir. Serializado por mutex em memória do processo.
    /// </summary>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Try to acquire the lock without blocking indefinitely — return 409 if another backfill is in progress.
        if (!await BackfillLock.WaitAsync(TimeSpan.Zero, cancellationToken))
        {
            logger.LogWarning("Backfill requested while another backfill is running; rejecting with 409.");
            return Conflict(new { error = "Outro backfill está em execução. Tente novamente em instantes." });
        }

        try
        {
            return await RunBackfillAsync(effectiveLimit, cancellationToken);
        }
        finally
        {
    ...
    private async Task<IActionResult> RunBackfillAsync(int effectiveLimit, CancellationToken cancellationToken)
    {
        var callerId = User.FindFirst(System.Security.Claims.ClaimTypes.NameIdentifier)?.Value ?? "unknown";
        logger.LogInformation("[AUDIT] Clinical backfill started. CallerUserId={CallerUserId} Limit={Limit}", callerId, effectiveLimit);
