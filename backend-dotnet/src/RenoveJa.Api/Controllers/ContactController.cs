...
public class ContactController : ControllerBase
{
    ...
    public async Task<IActionResult> Post([FromBody] ContactRequest request, CancellationToken ct)
    {
        ...
        try
        {
            ...
            // Mask local part of email for logging (preserve first 2 chars of local part when possible)
            string maskedEmail;
            ...
            {
                // Local part is 1-2 chars — fully mask it
                maskedEmail = new string('*', atIdx) + email[atIdx..];
            }
            else
            {
                maskedEmail = email[..2] + new string('*', atIdx - 2) + email[atIdx..];
            }
