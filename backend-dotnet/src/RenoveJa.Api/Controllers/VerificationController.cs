    ...
    ILogger<VerificationController> logger) : ControllerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            // Bound _secretCode length to avoid passing arbitrary data downstream.
            if (_secretCode.Length > 32)
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                // SSRF mitigation: exigimos HTTPS e aplicamos timeout curto.
                if (bytes == null)
                {
                    if (!Uri.TryCreate(refOrUrl, UriKind.Absolute, out var parsedUri)
                        || parsedUri.Scheme != Uri.UriSchemeHttps)
                    {
                        logger.LogWarning("Verify: rejeitado refOrUrl inválido/inseguro para requestId={RequestId}", id);
                        return NotFound(new { error = "Documento não encontrado." });
                    }
                    using var client = httpClientFactory.CreateClient();
                    client.Timeout = TimeSpan.FromSeconds(30);
                    bytes = await client.GetByteArrayAsync(parsedUri, cancellationToken);
                }
            }
