    ...
    ILogger<DailyWebhookController> logger) : ControllerBase
{
    ...
    public async Task<IActionResult> Handle([FromBody] DailyWebhookPayload? payload, CancellationToken cancellationToken)
    {
...

        {
            ...
            // Guard: se por algum motivo o buffering não estiver ativo, falha fechada.
            if (!Request.Body.CanSeek)
            {
                logger.LogError("[DailyWebhook] Request.Body não é seekable — EnableBuffering() ausente? Rejeitando.");
                return Unauthorized();
            }
            ...
            {
                logger.LogWarning("[DailyWebhook] Raw body vazio — rejeitando.");
                return Unauthorized();
            }

            var keyBytes = Encoding.UTF8.GetBytes(configuredSecret);
        ...
        var recordingId = payload.Payload?.RecordingId;
        var roomName = payload.Payload?.RoomName;
        ...
        // Bug fix: Idempotência — se a gravação já foi processada para este request, não re-baixar/re-upload.
        // Daily pode reenviar o webhook em caso de falha ou timeout do consumidor; sem esta verificação,
        // processaríamos e pagaríamos egress/storage várias vezes pelo mesmo recording_id.
        var existingAnamnesis = await consultationAnamnesisRepository.GetByRequestIdAsync(requestId, cancellationToken);
        if (existingAnamnesis != null && !string.IsNullOrEmpty(existingAnamnesis.RecordingFileUrl))
        {
            logger.LogInformation(
                "[DailyWebhook] Recording já processado (idempotent skip) — RequestId={RequestId} RecordingId={RecordingId}",
                requestId, recordingId);
            // Garantir limpeza do recording no Daily mesmo em replays, para não deixar lixo acumulado.
            try { await dailyVideoService.DeleteRecordingAsync(recordingId, cancellationToken); }
            catch (Exception delEx) { logger.LogWarning(delEx, "[DailyWebhook] Idempotent replay: falha ao deletar recording {RecordingId}", recordingId); }
            return Ok(new { ignored = true, reason = "already_processed" });
        }
        ...
        for (var attempt = 1; attempt <= maxAttempts; attempt++)
        {
            ...
            try
            {
                ...
                // BUG FIX: HttpResponseMessage não era descartada após upload do gravado — vazava socket por gravação.
                using var response = await client.GetAsync(downloadLink, HttpCompletionOption.ResponseHeadersRead, cancellationToken);
        ...
        // Reuso do lookup de idempotência feito acima (existingAnamnesis); se outro thread criou no meio, refazemos.
        var existing = existingAnamnesis ?? await consultationAnamnesisRepository.GetByRequestIdAsync(requestId, cancellationToken);
        if (existing != null)
        {
    ...
    public class DailyWebhookPayload
    {
        ...
        public long? GetEventTimestamp() => EventTs;
    }
