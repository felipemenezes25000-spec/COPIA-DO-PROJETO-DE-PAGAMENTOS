    ...
    ILogger<ConsultationWorkflowController> logger) : ControllerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (serviceException != null)
        {
            switch (serviceException)
            {
                ...
                default:
                    logger.LogError(serviceException, "[FinishConsultation] Erro inesperado. RequestId={RequestId}", id);
                    return StatusCode(500, new { error = "Erro ao encerrar consulta." });
            }
        }
    ...
    public async Task<IActionResult> AutoFinishConsultation(Guid id, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                logger.LogWarning(signalEx, "[AutoFinish] Falha ao notificar SignalR ConsultationEnded para {RequestId}", id);
            }
            ...
            {
                logger.LogWarning(roomEx, "[AutoFinish] Falha ao iniciar deleção da sala Daily para {RequestId}", id);
            }
