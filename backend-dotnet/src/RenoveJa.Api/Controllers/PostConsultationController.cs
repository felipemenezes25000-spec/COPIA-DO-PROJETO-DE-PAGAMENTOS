    ...
    ILogger<PostConsultationController> logger) : ControllerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(500, new { error = "Erro ao buscar documentos. Tente novamente." });
        }
    }
        ...
        Guid documentId, [FromQuery] string? token, CancellationToken cancellationToken)
    {
        try
        {
            ...
            else
            {
                ...
                if (pdfBytes == null)
                {
                    // Guard contra http(s) não-https e SSRF: apenas https externos.
                    if (!Uri.TryCreate(pathOrUrl, UriKind.Absolute, out var parsedUrl)
                        || parsedUrl.Scheme != Uri.UriSchemeHttps)
                    {
                        logger.LogWarning("Invalid document URL scheme for {DocumentId}", documentId);
                        return NotFound(new { error = "Document not found." });
                    }
                    var httpClient = httpClientFactory.CreateClient();
                    httpClient.Timeout = TimeSpan.FromSeconds(30);
                    using var httpResp = await httpClient.GetAsync(parsedUrl, cancellationToken);
                    if (!httpResp.IsSuccessStatusCode)
                    {
                        logger.LogWarning("Document fetch failed with {Status} for {DocumentId}", (int)httpResp.StatusCode, documentId);
                        return NotFound(new { error = "Document not found." });
                    }
                    // Cap em 50MB para evitar exaustão de memória.
                    const long maxBytes = 50L * 1024 * 1024;
                    if (httpResp.Content.Headers.ContentLength is long len && len > maxBytes)
                        return StatusCode(413, new { error = "Document too large." });
                    pdfBytes = await httpResp.Content.ReadAsByteArrayAsync(cancellationToken);
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(500, new { error = "Erro ao baixar documento. Tente novamente." });
        }
    }
}
