...
{
    // Tamanho máximo aceito para um código de verificação (hash + alguns códigos legados).
    private const int MaxCodeLength = 128;

    /// <summary>
    /// Compara strings em tempo constante (mitiga timing attacks no fallback legado de access_code).
    /// </summary>
    private static bool FixedTimeEquals(string a, string b)
    {
        ...
        {
            // Ainda chama FixedTimeEquals para manter tempo estável, mas garante retorno false.
            _ = CryptographicOperations.FixedTimeEquals(bBytes, bBytes);
            return false;
        }
        ...
        CancellationToken ct)
    {
        ...
        if (request.Code.Length > MaxCodeLength)
            return BadRequest(new { status = "error", reason = "INVALID_CODE", message = "Código inválido." });
        ...
        try
        {
            ...
            {
                // Fallback: comparar access_code direto (legado) — em tempo constante (mitiga timing attack).
                if (!FixedTimeEquals(request.Code.Trim(), security.Value.accessCode.Trim()))
                    return Ok(new { status = "invalid", reason = "INVALID_CODE", message = "Código inválido." });
            }
            else
            {
        ...
        CancellationToken ct)
    {
        ...
        if (security.HasValue && !string.IsNullOrEmpty(security.Value.verifyCodeHash))
            ...
            codeValid = FixedTimeEquals(trimmed, security.Value.accessCode.Trim());
        ...
        try
        {
            ...
            else
            {
                ...
                if (pdfBytes == null)
                {
                    // SSRF mitigation: exige HTTPS absoluto e aplica timeout.
                    if (!Uri.TryCreate(pathOrUrl, UriKind.Absolute, out var parsedUri)
                        || parsedUri.Scheme != Uri.UriSchemeHttps)
                    {
                        logger.LogWarning("Documents: rejeitado pathOrUrl inválido/inseguro para documentId={DocumentId}", documentId);
                        return NotFound(new { error = "Documento não encontrado." });
                    }
                    var httpClient = httpClientFactory.CreateClient();
                    httpClient.Timeout = TimeSpan.FromSeconds(30);
                    pdfBytes = await httpClient.GetByteArrayAsync(parsedUri, ct);
                }
            }
        ...
        CancellationToken ct)
    {
        ...
        // Input bounds (prevent oversized payloads)
        if (request.PharmacyName.Length > 200
            || request.PharmacistName.Length > 200
            || request.PharmacistCrf.Length > 50)
            return BadRequest(new { error = "Dados da dispensação excedem o tamanho máximo." });
        ...
        try
        {
            ...
            if (security.HasValue && !string.IsNullOrEmpty(security.Value.verifyCodeHash))
                ...
                codeValid = FixedTimeEquals(request.Code.Trim(), security.Value.accessCode.Trim());
        ...
        {
            logger.LogError(ex, "Dispense-by-code DB error: DocumentId={DocumentId}, SqlState={SqlState}",
                documentId, ex.SqlState);
            return StatusCode(503, new { error = "Serviço temporariamente indisponível. Tente novamente em alguns segundos." });
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Dispense-by-code failed for document {DocumentId}: {ExType}",
        ...
        CancellationToken ct)
    {
        if (request is null)
            return BadRequest(new { error = "Corpo da requisição inválido." });
        if (string.IsNullOrWhiteSpace(request.PharmacistName))
            return BadRequest(new { error = "Informe o nome do(a) farmacêutico(a) ou responsável." });
        if (string.IsNullOrWhiteSpace(request.PharmacistCrf))
            return BadRequest(new { error = "Informe o CRF do(a) farmacêutico(a)." });
        if ((request.PharmacyName?.Length ?? 0) > 200
            || request.PharmacistName.Length > 200
            || request.PharmacistCrf.Length > 50)
            return BadRequest(new { error = "Dados da dispensação excedem o tamanho máximo." });
        ...
        {
            logger.LogError(ex, "Dispense DB error: DocumentId={DocumentId}, SqlState={SqlState}",
                documentId, ex.SqlState);
            return StatusCode(503, new { error = "Serviço temporariamente indisponível. Tente novamente em alguns segundos." });
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Dispense failed for document {DocumentId}: {ExType}",
                documentId, ex.GetType().Name);
            return BadRequest(new { error = "Erro ao registrar dispensação. Tente novamente." });
        }
    }
