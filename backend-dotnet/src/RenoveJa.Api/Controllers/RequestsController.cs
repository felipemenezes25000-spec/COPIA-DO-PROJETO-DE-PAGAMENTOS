    ...
    ILogger<RequestsController> logger) : ControllerBase
{
    ...
    public async Task<IActionResult> CreatePrescription(CancellationToken cancellationToken)
    {
        var userId = GetUserId();
            ...
            // BUG FIX: caminho JSON podia chegar com PrescriptionType null/whitespace,
            // estourando na camada de domínio. Validamos explicitamente aqui.
            if (string.IsNullOrWhiteSpace(request.PrescriptionType))
                return BadRequest(new { error = "Campo 'prescriptionType' é obrigatório (simples ou controlado)." });
    ...
    public async Task<IActionResult> CreateExam(CancellationToken cancellationToken)
    {
        ...
        if (Request.HasFormContentType)
        {
            ...
            if (Request.Form.Files.Count > 0)
            {
                ...
                foreach (var file in Request.Form.Files)
                {
                    ...
                    if (file.Length > 5 * 1024 * 1024)
                        return BadRequest(new { error = $"Arquivo {file.FileName} excede 5 MB." });
                    // BUG FIX: quando `file.ContentType` era null, caía em "image/jpeg" sem
                    // verificar — um upload sem content-type passava como JPEG, contornando
                    // o allowlist. Agora rejeitamos explicitamente quando ausente.
                    var contentType = file.ContentType;
                    if (string.IsNullOrWhiteSpace(contentType))
                        return BadRequest(new { error = $"Tipo do arquivo {file.FileName} não informado. Use: {string.Join(", ", AllowedImageContentTypes)}" });
                    if (!AllowedImageContentTypes.Contains(contentType, StringComparer.OrdinalIgnoreCase))
        ...
        else
        {
            ...
            // BUG FIX: validação null/vazio no caminho JSON (multipart já valida acima).
            // Antes, um body JSON sem examType ou sem exams/symptoms/images chegava ao service.
            if (string.IsNullOrWhiteSpace(bodyRequest.ExamType))
                return BadRequest(new { error = "Campo 'examType' é obrigatório." });

            var hasExams = bodyRequest.Exams != null && bodyRequest.Exams.Any(e => !string.IsNullOrWhiteSpace(e));
            var hasSymptoms = !string.IsNullOrWhiteSpace(bodyRequest.Symptoms);
            var hasImages = examImgCount > 0;
            if (!hasExams && !hasSymptoms && !hasImages)
                return BadRequest(new { error = "Informe pelo menos um exame, imagens do pedido ou sintomas/indicação." });
