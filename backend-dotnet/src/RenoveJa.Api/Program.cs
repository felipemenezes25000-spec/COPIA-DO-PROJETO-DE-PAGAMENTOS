...
builder.Services.AddSwaggerGen(options =>
{
    options.SwaggerDoc("v1", new OpenApiInfo
    {
        Title = "RenoveJá API",
        Version = "v1",
        Description = "API de telemedicina: receitas, exames, consultas por vídeo e assinatura digital ICP-Brasil.",
...
builder.Services.AddCors(options =>
{
    ...
    // IMPORTANTE: o portal RH (rh.renovejasaude.com.br) precisa estar aqui porque se por
    // qualquer razão o appsettings.Production.json não carregar no ECS (SSM override, deploy
    // sem o arquivo, environment errada), o Monitor de Produtividade Médica cai com CORS
    // block em TODAS as 4 páginas (Produtividade / Fila ao vivo / Relatórios / Precificação).
    // Duplicar aqui é inofensivo: MergeCorsOrigins deduplica com HashSet case-insensitive.
    ...
    // Policy para endpoints de verificação — libera hosts oficiais do ITI (validador ICP-Brasil)
    // E o site público do RenoveJá (frontend de verificação por código). Sem credentials,
    // com origin checada por hostname exato (parse de Uri) para impedir bypass por substring
    // (ex.: "https://evil.com/validar.iti.gov.br" ou "https://validar.iti.gov.br.evil.com").
    options.AddPolicy("VerifyCors", policy =>
    {
        static bool IsVerifyOrigin(string? origin)
        {
            if (string.IsNullOrEmpty(origin)) return false;
            try
            {
                ...
                var host = uri.Host;
                // Validador ICP-Brasil (ITI)
                if (host.Equals("validar.iti.gov.br", StringComparison.OrdinalIgnoreCase)
                    || host.Equals("h-validar.iti.gov.br", StringComparison.OrdinalIgnoreCase)
                    || host.Equals("www.validar.iti.gov.br", StringComparison.OrdinalIgnoreCase)
                    || host.EndsWith(".iti.gov.br", StringComparison.OrdinalIgnoreCase))
                    return true;
                // Site público RenoveJá (frontend de verificação por código)
                if (host.Equals("renovejasaude.com.br", StringComparison.OrdinalIgnoreCase)
                    || host.Equals("www.renovejasaude.com.br", StringComparison.OrdinalIgnoreCase))
                    return true;
                return false;
            }
            catch { return false; }
        ...
        policy.SetIsOriginAllowed(IsVerifyOrigin)
...
builder.Services.AddRateLimiter(options =>
{
    ...
    // Quando o rate limiter rejeita uma requisição cross-origin, o 429 sai SEM headers
    // CORS — o browser então reporta "Erro de CORS" no DevTools (mascarando o 429 real)
    // e a UI cai em "Erro de conexão". Aqui re-injetamos os headers CORS para a whitelist
    // do site público (renovejasaude.com.br + iti) e respondemos preflight com 204.
    options.OnRejected = async (context, cancellationToken) =>
    {
        ...
        if (!string.IsNullOrEmpty(origin))
        {
            try
            {
                ...
                var allowed = isHttps && (
                    host.Equals("renovejasaude.com.br", StringComparison.OrdinalIgnoreCase)
                    || host.Equals("www.renovejasaude.com.br", StringComparison.OrdinalIgnoreCase)
                    || host.Equals("admin.renovejasaude.com.br", StringComparison.OrdinalIgnoreCase)
                    || host.Equals("app.renovejasaude.com.br", StringComparison.OrdinalIgnoreCase)
                    || host.Equals("medico.renovejasaude.com.br", StringComparison.OrdinalIgnoreCase)
                    || host.Equals("rh.renovejasaude.com.br", StringComparison.OrdinalIgnoreCase)
                    || host.EndsWith(".iti.gov.br", StringComparison.OrdinalIgnoreCase));
                if (allowed)
                {
                    http.Response.Headers["Access-Control-Allow-Origin"] = origin;
                    http.Response.Headers["Vary"] = "Origin";
                    http.Response.Headers["Access-Control-Allow-Methods"] = "GET, POST, OPTIONS";
                    http.Response.Headers["Access-Control-Allow-Headers"] = "Content-Type, Authorization";
                }
            }
        ...
        await http.Response.WriteAsJsonAsync(
            new { error = "Muitas requisições. Aguarde alguns segundos e tente novamente." },
            cancellationToken);
    };
    ...
    // Limiter para verificação pública: 30 req/min por IP.
    // Antes era 5/min, mas a página /verify do site público chama 2 endpoints por
    // submissão (/api/documents/verify + /api/prescriptions/verify), então 3 tentativas
    // (totalmente normais para um usuário digitando o código errado uma ou duas vezes)
    // já estouravam o limite. O 429 saía sem CORS headers e o Chrome reportava como
    // "Erro de CORS", confundindo o diagnóstico. 30/min ainda é bem restritivo contra
    // brute-force (códigos têm 6 dígitos = 1M combinações; em 30/min levaria ~23 dias).
    // Preflight OPTIONS é exempto: responder a preflight não pode ser rate-limitado, ou
    // o browser perde até a chance de fazer a requisição real.
    options.AddPolicy("verify", httpContext =>
    {
        if (HttpMethods.IsOptions(httpContext.Request.Method))
            return RateLimitPartition.GetNoLimiter("verify-preflight");
    ...
    // Limiter para registro: 20 req/min
    // Bumped from 10 → 20 para acomodar onboarding em lote do RH (ex.: recrutador
    // cadastrando 5-10 médicos em sequência + upload de documentos logo depois
    // — cada médico consome 2 requests no register bucket: POST /doctors/from-hr
    // + POST /doctors/from-hr/{userId}/documents). 20/min/IP ainda é baixo o
    // suficiente para barrar abuso automatizado.
    ...
    // Limiter para endpoints de IA (OpenAI/Gemini): 10 req/min por usuário (ou IP).
    // Protege contra abuso de chave OpenAI / cost blowup. Particionado por userId quando autenticado.
...
// IMPORTANTE: MigrationRunner curto-circuita sozinho quando DatabaseUrl não está
// configurada (cenário de dev/local sem banco), então qualquer exceção que chegar
// aqui é um erro REAL de schema. Propagamos para abortar o startup — é
// preferível o app não subir do que subir com schema incompleto e servir 500s
// silenciosos em prod (incidente 2026-04-09: productivity endpoints quebrados
// porque product_prices/doctor_contracts falharam em migration e foram logadas
// apenas como Warning).
...
// Seed do admin do painel RH (idempotente). Usa ADMIN_SEED_EMAIL/ADMIN_SEED_PASSWORD.
// Só precisa rodar uma vez por ambiente — é seguro manter sempre no startup.
try
{
    var seederLogger = app.Services.GetRequiredService<ILogger<Program>>();
    await RenoveJa.Api.Services.AdminSeeder.RunAsync(app.Services, seederLogger);
}
catch (Exception ex)
{
    Log.Warning(ex, "Falha ao executar AdminSeeder (API continua subindo)");
}
...
// Swagger habilitado APENAS em Development. Em qualquer outro ambiente fica desabilitado
// para não expor a superfície completa da API publicamente (mesmo com Authorize).
if (app.Environment.IsDevelopment())
{
    app.UseSwagger();
    app.UseSwaggerUI();
...
// Exception handling deve envolver TODO o pipeline — colocado antes de qualquer
// middleware que possa lançar, inclusive o de correlação (que roda dentro).
app.UseMiddleware<ExceptionHandlingMiddleware>();

// Correlation ID precisa vir antes do logger e do rate limiter para que 429 e logs
// carreguem o X-Correlation-Id corretamente.
app.UseMiddleware<CorrelationIdMiddleware>();
app.UseMiddleware<ApiRequestLoggingMiddleware>();

// UseRouting é obrigatório ANTES de UseCors/UseRateLimiter/UseAuthorization para que
// middlewares endpoint-aware (ex.: [EnableRateLimiting("auth")], [EnableCors(...)])
// consigam ler a metadata do endpoint resolvido. Sem isso, políticas por endpoint
// são silenciosamente ignoradas.
app.UseRouting();

// CORS entre UseRouting e UseAuthorization (ASP.NET Core docs).
if (app.Environment.IsDevelopment())
