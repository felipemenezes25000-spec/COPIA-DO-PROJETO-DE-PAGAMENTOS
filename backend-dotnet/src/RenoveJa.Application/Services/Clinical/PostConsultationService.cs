...
{
    // ── Rate limiter (in-memory) para tentativas de senha do certificado digital
    // por médico, com backoff exponencial. Mitiga brute-force quando o atacante
    // já controla a sessão do médico mas não conhece a senha do PFX.
    private static readonly ConcurrentDictionary<Guid, (int FailCount, DateTimeOffset NextAllowedAt)> CertPwdAttempts = new();

    private void EnsureCertificatePasswordAttemptAllowed(Guid doctorId)
    {
        if (CertPwdAttempts.TryGetValue(doctorId, out var state))
        {
            var now = DateTimeOffset.UtcNow;
            if (now < state.NextAllowedAt)
            {
                var waitSeconds = (int)Math.Ceiling((state.NextAllowedAt - now).TotalSeconds);
    ...
    private void RegisterCertificatePasswordFailure(Guid doctorId)
    {
        CertPwdAttempts.AddOrUpdate(
            doctorId,
            _ =>
            {
                var delay = Math.Min(1 << 1, 60); // 2^1 = 2s no primeiro fail
                return (1, DateTimeOffset.UtcNow.AddSeconds(delay));
            },
            (_, prev) =>
            {
                var newCount = prev.FailCount + 1;
                // Math.Pow para evitar overflow em counts grandes; clamp 60s
                var delay = Math.Min(Math.Pow(2, newCount), 60);
                return (newCount, DateTimeOffset.UtcNow.AddSeconds(delay));
            });
        logger.LogWarning(
            "Certificate password validation failed for doctor {DoctorId}", doctorId);
    }

    private static void ClearCertificatePasswordFailures(Guid doctorId)
    {
        CertPwdAttempts.TryRemove(doctorId, out _);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Rate-limit por médico (anti brute-force) — backoff exponencial até 60s
        EnsureCertificatePasswordAttemptAllowed(doctorUserId);
        ...
        // FinalizeEncounterAsync refaz o fetch do encounter e aplica UpdateClinicalNotes antes de persistir,
        // então não mutamos o objeto local aqui (evita confusão com dados que nunca seriam salvos).
