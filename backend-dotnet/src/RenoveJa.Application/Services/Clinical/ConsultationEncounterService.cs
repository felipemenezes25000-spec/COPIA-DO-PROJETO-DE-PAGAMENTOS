    ...
    ILogger<ConsultationEncounterService> logger) : IConsultationEncounterService
{
    ...
    {
        // Garantir que patient_profiles existe e obter o patients.id (PK da tabela patients)
    ...
    private async Task<Patient> EnsurePatientFromUserAsync(Guid userId, CancellationToken cancellationToken)
    {
        ...
        {
            // Race: outra requisição concorrente já criou o Patient — recupera o existente.
            logger.LogInformation(
                "[ConsultationEncounter] Race na criação do Patient para userId {UserId}, recuperando existente.",
                userId);
