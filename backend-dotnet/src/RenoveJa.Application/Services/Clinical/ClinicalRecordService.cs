    ...
    ILogger<ClinicalRecordService> logger) : IClinicalRecordService
{
    public async Task<Patient> EnsurePatientFromUserAsync(Guid userId, CancellationToken cancellationToken = default)
    {
        ...
        {
            // Race: outra requisição concorrente criou o Patient entre o GetByUserIdAsync e o CreateAsync.
            // Recupera o registro existente ao invés de propagar o erro 23505.
            logger.LogInformation(
                "EnsurePatientFromUserAsync: race detectada na criação do Patient para userId {UserId}, recuperando existente.",
                userId);
            var afterRace = await patientRepository.GetByUserIdAsync(userId, cancellationToken);
    ...
    {
        // Detecta violação de unicidade em Postgres/Supabase sem acoplar a Npgsql no projeto Application.
    ...
    public async Task<PatientSummaryDto> GetPatientSummaryAsync(Guid userId, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            return new PatientSummaryDto
            {
                ...
                {
                    // Total de "solicitações" engloba documentos clínicos + encounters (fonte clínica),
                    // não apenas documentos. Antes reportava documents.Count duplicando o TotalPrescriptions/Exams.
                    TotalRequests = clinicalTotal,
