    ...
    IOptions<DailyConfig> dailyConfig) : IVideoService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (dto.RequestId == Guid.Empty)
            throw new ArgumentException("RequestId é obrigatório.", nameof(dto));
        ...
        {
            // Race condition: outra requisição concorrente pode ter criado a sala
            // entre o GetByRequestIdAsync acima e o CreateAsync. Constraint unique
            // em request_id lança — tentamos obter a sala existente antes de propagar.
            var concurrent = await videoRoomRepository.GetByRequestIdAsync(dto.RequestId, cancellationToken);
            if (concurrent != null)
