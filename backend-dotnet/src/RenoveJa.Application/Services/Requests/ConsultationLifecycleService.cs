    ...
    ILogger<ConsultationLifecycleService> logger) : IConsultationLifecycleService
{
    ...
    private sealed class LockEntry
    {
        ...
        // Stored as ticks (UTC) and updated atomically via Interlocked for thread safety.
        ...
        public void Touch() => Interlocked.Exchange(ref _lastAccessedTicks, DateTime.UtcNow.Ticks);
    }

    private static readonly ConcurrentDictionary<Guid, LockEntry> _consultationLocks = new();
    private static long _lastCleanupTicks = DateTime.UtcNow.Ticks;

    // FIX: Statuses from which StartConsultation may transition to InConsultation.
    // Used by both the idempotency check and the transition guard so they stay in sync.
    private static readonly HashSet<RequestStatus> _validStartStatuses = new()
    {
        RequestStatus.Paid,
    ...
    {
        var entry = _consultationLocks.GetOrAdd(consultationId, _ => new LockEntry());
        entry.Touch();
        CleanupStaleLocks();
    ...
    private static void CleanupStaleLocks()
    {
        ...
        if ((now - new DateTime(lastTicks, DateTimeKind.Utc)).TotalMinutes < 10) return;
        // Only one thread should perform the sweep per interval.
        if (Interlocked.CompareExchange(ref _lastCleanupTicks, now.Ticks, lastTicks) != lastTicks) return;

        var cutoffTicks = now.AddMinutes(-5).Ticks;
        foreach (var kvp in _consultationLocks)
        {
            // Remove entries older than 5 minutes regardless of lock state.
            // If the semaphore is currently held, it will be re-created on next access (rare race).
            if (kvp.Value.LastAccessedTicks < cutoffTicks)
            {
                if (_consultationLocks.TryRemove(kvp.Key, out var removed))
                {
                    try { removed.Semaphore.Dispose(); } catch { /* ignore */ }
                }
            }
        }
    }
    ...
    public async Task<RequestResponseDto> StartConsultationAsync(Guid id, Guid doctorId, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (!_validStartStatuses.Contains(request.Status))
                throw new InvalidOperationException($"Consultation can only be started when status is one of [{string.Join(", ", _validStartStatuses)}]. Current status: {request.Status}.");
