    ...
    ILogger<RequestQueryService> logger) : IRequestQueryService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (user?.Role == UserRole.Doctor)
        {
            ...
            // Force materialization of the concat before further LINQ to avoid
            // multiple enumerations of the source collections.
            var combined = doctorRequests.Concat(available).ToList();
            requests = combined
                .DistinctBy(r => r.Id)
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var specialty = doctorProfile?.Specialty;
        var requests = await requestRepository.ListAiRejectedBySpecialtyAsync(specialty, limit: 100, cancellationToken);

        return requests.Select(r => RequestHelpers.MapRequestToDto(r, _apiBaseUrl, documentTokenService)).ToList();
    }
