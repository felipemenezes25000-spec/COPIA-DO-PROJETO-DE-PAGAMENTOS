    ...
    ILogger<SignatureService> logger) : ISignatureService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Só permite assinar se o pedido foi aprovado (status Paid = aprovado)
        ...
        if (request.DoctorId.HasValue)
        {
...

            {
                // Senha do PFX obrigatória no fluxo automático de assinatura
                    ...
                    // Senha do PFX inválida é um erro operacional comum e não deve cair no fallback
                    // genérico do middleware em produção — lançamos uma exceção tipada para que o
                    // ExceptionHandlingMiddleware retorne a mensagem real ao médico.
                    if (signErrorMessage.Contains("Senha do certificado", StringComparison.OrdinalIgnoreCase))
                    {
                        throw new SignatureInvalidPfxPasswordException(signErrorMessage);
                    }
    ...
    public async Task<(bool IsValid, IReadOnlyList<string> MissingFields, IReadOnlyList<string> Messages)> ValidatePrescriptionAsync(Guid id, Guid userId, CancellationToken cancellationToken = default)
    {
        ...
        if (!isDoctor && !isPatient)
            throw new UnauthorizedAccessException("Você não tem permissão para validar este documento.");
