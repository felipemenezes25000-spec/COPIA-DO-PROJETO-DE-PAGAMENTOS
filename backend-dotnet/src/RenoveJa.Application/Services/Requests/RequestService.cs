...
public class RequestService(
    ...
    IRequestRoutingStrategy routingStrategy,
    ILogger<RequestService> logger) : IRequestService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var latest = await requestRepository.GetByIdAsync(medicalRequest!.Id, cancellationToken)
            ?? throw new InvalidOperationException($"Receita {medicalRequest.Id} não encontrada após criação. Inconsistência no repositório.");
        var req = latest;
    ...
    /// <summary>
    /// Cria uma solicitação de exame. Status Submitted.
    /// </summary>
    public async Task<RequestResponseDto> CreateExamAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        // Valida indicação clínica e demais regras antes de tocar no repositório.
        // Espelha o que CreateConsultationAsync já fazia (gap fechado).
        var validator = new CreateExamRequestValidator();
        ...
        var latest = await requestRepository.GetByIdAsync(medicalRequest!.Id, cancellationToken)
            ?? throw new InvalidOperationException($"Exame {medicalRequest.Id} não encontrado após criação. Inconsistência no repositório.");
        var req = latest;
    ...
    /// <summary>
    /// Cria uma solicitação de consulta. Status SearchingDoctor.
    /// </summary>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Sem DoctorId atribuído, NENHUM médico pode mexer no status (evita bypass de PII).
        ...
        {
            // Guard: rejeição só pode partir de estados válidos. O Reject() do domínio
            // valida internamente, mas conferimos antes para não corromper estado caso
            // UpdateStatus tenha sido chamado.
            if (request.Status != RequestStatus.InReview && request.Status != RequestStatus.Submitted &&
                request.Status != RequestStatus.SearchingDoctor)
                throw new InvalidOperationException($"Não é possível rejeitar uma solicitação no status {request.Status}.");
    ...
    /// <summary>
    /// Atribui a solicitação ao médico disponível mais adequado, usando
    /// <see cref="IRequestRoutingStrategy"/> (balanceamento de carga + filtro
    /// estrito por especialidade). Se nenhum médico satisfizer o filtro, a
    /// solicitação fica na fila aguardando (NÃO faz fallback cross-specialty).
    /// </summary>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var decision = await routingStrategy.SelectDoctorAsync(routingContext, cancellationToken);
        if (decision is null)
        {
            // Nenhum médico apto: mantém a solicitação na fila. O worker de SLA
            // (Phase E) ou o próximo AssignToQueueAsync tentará novamente.
            logger.LogInformation(
                "Solicitação {RequestId} permanecerá aguardando — nenhum médico disponível para specialty={Specialty}",
                request.Id,
                request.RequiredSpecialty ?? "(qualquer)");
            throw new InvalidOperationException("No available doctors");
        }

        request.AssignDoctor(decision.DoctorUserId, decision.DoctorName);
        request = await requestRepository.UpdateAsync(request, cancellationToken);

        // Marca o médico como "acabou de receber uma atribuição" — o próximo pedido
        // vai preferencialmente para outro médico (rotação justa).
        await doctorRepository.UpdateLastAssignedAtAsync(
            decision.DoctorProfileId,
            DateTime.UtcNow,
            cancellationToken);

        // Notificações (paciente + médico)
        await pushDispatcher.SendAsync(
            PushNotificationRules.InReview(request.PatientId, request.Id, request.RequestType),
            cancellationToken);
        await pushDispatcher.SendAsync(
            PushNotificationRules.RequestAssigned(decision.DoctorUserId, request.Id, request.RequestType),
            cancellationToken);
    ...
    public async Task<RequestResponseDto> UpdatePrescriptionContentAsync(Guid id, List<string>? medications, string? notes, Guid doctorId, CancellationToken cancellationToken = default, string? prescriptionKind = null)
    {
        var request = await requestRepository.GetByIdAsync(id, cancellationToken);
        if (request == null) throw new KeyNotFoundException("Solicitação não encontrada");
        // BUG FIX: sem DoctorId atribuído, ninguém pode editar (evita bypass de PII).
        if (!request.DoctorId.HasValue || request.DoctorId.Value != doctorId)
            throw new UnauthorizedAccessException("Somente o médico atribuído pode atualizar.");
        ...
        MedicalRequest request)
    {
        ...
        // Receitas podem ter nomes parciais, abreviados ou ilegíveis.
    ...
    private async Task RunPrescriptionAiAndUpdateAsync(MedicalRequest medicalRequest, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                case "reject":
                    medicalRequest.RejectByAi(outcome.RejectionMessage!);
