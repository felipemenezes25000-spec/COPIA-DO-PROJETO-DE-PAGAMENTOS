...
#pragma warning restore CS9113
{
        ...
        Guid patientId, List<string> medications, CancellationToken ct)
    {
        ...
        // BUG FIX: the previous bidirectional Contains match produced false positives
        // (e.g. "amol" ⊂ "paracetamol" or "tal" ⊂ "cetoconazol tópico"). Compare on
        // normalized word tokens so only medications that share a meaningful word match.
        static HashSet<string> Tokenize(string s)
        {
            return s
                .Split(new[] { ' ', ',', ';', '\t', '/', '(', ')', '-' }, StringSplitOptions.RemoveEmptyEntries)
                .Where(t => t.Length >= 4) // skip dosage units / short tokens
                .ToHashSet(StringComparer.OrdinalIgnoreCase);
        }

        var existingTokenSets = recentMeds.Select(m => (Name: m, Tokens: Tokenize(m))).ToList();

        foreach (var med in medications)
        {
            if (string.IsNullOrWhiteSpace(med)) continue;
            var normalized = med.ToLowerInvariant().Trim();
            var inputTokens = Tokenize(normalized);
            if (inputTokens.Count == 0) continue;

            var match = existingTokenSets.FirstOrDefault(e => e.Tokens.Overlaps(inputTokens));
            if (!string.IsNullOrEmpty(match.Name))
            {
                warnings.Add(new DuplicateWarning(
                    "medication", med,
                    $"Paciente já possui receita ativa com '{match.Name}' nos últimos {CooldownDays} dias."));
            }
        }
