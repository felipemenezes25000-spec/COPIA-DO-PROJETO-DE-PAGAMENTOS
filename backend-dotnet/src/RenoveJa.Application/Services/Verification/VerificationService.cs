    ...
    ILogger<VerificationService> logger) : IVerificationService
{
    ...
    public bool ValidateAccessCode(Guid requestId, string accessCode)
    {
        ...
        // BUG FIX: FixedTimeEquals throws ArgumentException when arrays differ in length.
        // Length check is constant-time (same value for any wrong-length input) and returns false without leaking timing info about the expected value.
    ...
    public static bool ValidateAccessCode(string? storedCode, string accessCode)
    {
        ...
        // BUG FIX: FixedTimeEquals throws ArgumentException on length mismatch.
