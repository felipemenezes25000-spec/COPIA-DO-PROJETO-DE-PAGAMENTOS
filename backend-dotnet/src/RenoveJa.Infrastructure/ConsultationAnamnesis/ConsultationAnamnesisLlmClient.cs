...
public sealed class ConsultationAnamnesisLlmClient
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        using var response = await client.PostAsync($"{apiBaseUrl}/chat/completions", requestContent, cancellationToken);
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            if (usedOpenAi && !string.IsNullOrEmpty(geminiKey))
            {
                ...
                using var fallbackResponse = await client.PostAsync($"{geminiUrl}/chat/completions", fallbackContent, cancellationToken);
