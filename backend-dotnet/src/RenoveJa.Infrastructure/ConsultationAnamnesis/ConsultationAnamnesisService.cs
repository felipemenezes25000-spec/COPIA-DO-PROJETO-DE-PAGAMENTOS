...
public class ConsultationAnamnesisService : IConsultationAnamnesisService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // LGPD: redigir PII (CPF, RG, telefone, email) antes de enviar ao LLM externo.
        // Preservamos o conteúdo clínico — apenas proteção de dados pessoais do paciente.
        processedTranscript = PromptSanitizer.RedactPiiOnly(processedTranscript);
        var sanitizedPreviousAnamnesis = string.IsNullOrWhiteSpace(previousAnamnesisJson)
            ? previousAnamnesisJson
            : PromptSanitizer.RedactPiiOnly(previousAnamnesisJson);
        ...
        var userContent = AnamnesisPrompts.BuildUserContentForAnamnesisV2(processedTranscript, sanitizedPreviousAnamnesis);
        ...
        try
        {
            ...
            // BUG FIX: GetProperty("choices") lançava KeyNotFoundException em respostas
            // degradadas (ex.: payload de erro). Valida cada nó antes de acessar.
            content = null;
