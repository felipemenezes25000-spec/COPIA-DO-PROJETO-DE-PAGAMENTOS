...
public class SoapNotesService : ISoapNotesService
{
    ...
    {
        // LGPD: Transcript vem do Whisper/Deepgram e pode conter CPF, RG, telefone, email
        // falados pelo paciente. Aplicamos RedactPiiOnly (sem truncar 10k nem mexer em markdown)
        // para preservar conteúdo clínico, apenas protegendo dados pessoais.
        var sanitizedTranscript = PromptSanitizer.RedactPiiOnly(transcript);
        var sanitizedAnamnesis = string.IsNullOrWhiteSpace(anamnesisJson)
            ? null
            : PromptSanitizer.RedactPiiOnly(anamnesisJson);

        var sb = new StringBuilder();
        sb.AppendLine("=== TRANSCRIPT DA TELECONSULTA ===");
        sb.AppendLine(sanitizedTranscript.Length > 12000 ? sanitizedTranscript[..12000] + "\n[... truncado ...]" : sanitizedTranscript);

        if (!string.IsNullOrWhiteSpace(sanitizedAnamnesis))
        {
            sb.AppendLine("\n=== ANAMNESE ESTRUTURADA (use como referência) ===");
            sb.AppendLine(sanitizedAnamnesis.Length > 3000 ? sanitizedAnamnesis[..3000] + "..." : sanitizedAnamnesis);
        }
