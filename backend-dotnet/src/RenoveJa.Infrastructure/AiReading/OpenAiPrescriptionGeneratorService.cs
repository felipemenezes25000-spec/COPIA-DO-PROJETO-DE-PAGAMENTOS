...
public class OpenAiPrescriptionGeneratorService : IAiPrescriptionGeneratorService
{
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            // BUG FIX: GetProperty/[0] lançavam KeyNotFoundException/IndexOutOfRange
            // quando a API retornava formato inesperado (ex.: rate limit, erro filtrado).
            if (!doc.RootElement.TryGetProperty("choices", out var choicesEl)
                || choicesEl.ValueKind != JsonValueKind.Array
                || choicesEl.GetArrayLength() == 0)
            {
                _logger.LogWarning("IA prescrição: resposta sem 'choices' válido.");
                return null;
            }
            if (!choicesEl[0].TryGetProperty("message", out var msgEl)
                || !msgEl.TryGetProperty("content", out var contentEl))
            {
                _logger.LogWarning("IA prescrição: resposta sem 'message.content'.");
                return null;
            }
            var messageContent = contentEl.GetString();
