...
public class PrescriptionPdfService : IPrescriptionPdfService
{
    ...
    // Recado institucional fixo no rodapé de toda receita (req. Carolina Akiko 2026-04-07).
    // Posicionamento: RenoveJá+ é COMPLEMENTAR ao atendimento presencial — empurra paciente de volta à consulta de rotina.
    private const string PatientReminderMessage =
        "Receita renovada segundo normas de uso contínuo. Por gentileza, não se esqueça de retornar à consulta de rotina.";

    // Aviso institucional fixo em todo pedido de exame (req. Carolina Akiko 2026-04-07).
    // Reforça que exames de baixo risco emitidos via plataforma têm finalidade diagnóstica clara.
    private const string ExamPurposeNotice =
        "Exame solicitado para complementação ou investigação diagnóstica conforme avaliação clínica do médico assistente.";
    // Logo
    ...
    private static byte[]? LoadLogoImage()
    {
        ...
        lock (_logoLock)
        {
            ...
            catch { return null; } // TODO(logging): inject ILogger to log logo-load failures
        }
    }
        ...
        try
        {
            ...
            using (var ms = new MemoryStream())
            {
                ...
                using (var pdf = new PdfDocument(writer))
                {
                    ...
                    // Clinical indication — quando o paciente não informa motivo, o app envia
                    // "Investigação diagnóstica" como fallback (Carolina Akiko 2026-04-08).
                    // Nesse caso, suprimimos a seção para evitar redundância com o aviso
                    // institucional de finalidade do exame logo abaixo.
                    if (!string.IsNullOrWhiteSpace(data.ClinicalIndication) &&
                        !data.ClinicalIndication.Trim().Equals("Investigação diagnóstica", StringComparison.OrdinalIgnoreCase))
                    {
                    ...
                    // Aviso institucional de finalidade do exame
                    AddExamPurposeNotice(doc, fi);

                    // Legal
                    AddLegalFooter(doc, data.DoctorName, data.EmissionDate, f, fi);

                    // IMPORTANTE: fechar o Document antes de ler ms.ToArray() — sem Close() explícito o iText7
                    // não descarrega o xref/trailer no MemoryStream e o PDF retorna truncado (bug que fazia o
                    // app mobile mostrar "Erro ao processar o PDF. Tente novamente." no preview do exame).
                    doc.Close();
                }
                pdfBytes = ms.ToArray();
            }
    ...
    /// <summary>
    /// Recado institucional ao paciente (fixo, em letra pequena) — usado em toda receita.
    /// Não deve aparecer em pedidos de exame.
    /// </summary>
    private static void AddPatientReminderNote(Document doc, PdfFont fi)
    {
        doc.Add(new Paragraph(PatientReminderMessage)
            .SetFont(fi)
            .SetFontSize(6.5f)
            .SetFontColor(TextLight)
    ...
    /// <summary>
    /// Aviso institucional fixo em pedidos de exame (finalidade complementar/investigativa).
    /// Renderizado em letra pequena antes do AddLegalFooter — apenas no PDF de exame.
    /// </summary>
    private static void AddExamPurposeNotice(Document doc, PdfFont fi)
    {
        doc.Add(new Paragraph(ExamPurposeNotice)
            .SetFont(fi).SetFontSize(6.5f).SetFontColor(TextLight)
    ...
    {
        // BUG FIX: Guid.GetHashCode() NÃO é estável entre processos/execuções no .NET Core+.
        // Usávamos isso como "código de verificação" no PDF, então dois runs diferentes do backend
        // geravam códigos diferentes para o mesmo requestId — o paciente digitava o código
        // impresso na receita e o sistema rejeitava. Trocado por hash determinístico (SHA256).
        Span<byte> buf = stackalloc byte[16];
        if (!requestId.TryWriteBytes(buf))
            buf = requestId.ToByteArray();
        Span<byte> digest = stackalloc byte[32];
        System.Security.Cryptography.SHA256.HashData(buf, digest);
        // Pega 4 bytes (uint) como número base e reduz ao intervalo 000000..999999.
        uint n = (uint)(digest[0] << 24 | digest[1] << 16 | digest[2] << 8 | digest[3]);
    ...
    private static byte[]? GenerateQrCode(string data)
    {
        ...
        catch { return null; } // TODO(logging): inject ILogger to log QR code generation failures
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            using var ms = new MemoryStream();
            // BUG FIX: writer/pdf/document não estavam em using — em caso de exceção antes do
            // document.Close() abaixo, os handles nativos do iText ficavam pendurados até o GC.
            using var writer = new PdfWriter(ms);
            using var pdf = new PdfDocument(writer);
