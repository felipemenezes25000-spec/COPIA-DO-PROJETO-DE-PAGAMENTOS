    ...
    ILogger<S3StorageService> logger) : IStorageService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        for (var attempt = 1; attempt <= maxAttempts; attempt++)
        {
            try
            {
                ...
                // BUG FIX: MemoryStream era vazado a cada tentativa de upload.
        ...
        string path, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            // BUG FIX: GetObjectResponse implementa IDisposable (libera o stream HTTP
            // para o pool de conexões). Antes vazava socket/stream em cada download.
    ...
    public Task<string?> CreateSignedUrlAsync(
        string path, int expiresInSeconds = 300,
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            // SECURITY: limita TTL máximo a 1 hora. Sem esse clamp um caller podia gerar URLs
            // assinadas válidas por dias, contornando controles de acesso ao documento.
            const int MaxExpiresSeconds = 3600;
            if (expiresInSeconds <= 0) expiresInSeconds = 300;
            if (expiresInSeconds > MaxExpiresSeconds) expiresInSeconds = MaxExpiresSeconds;
    ...
    public string? ExtractPathFromStorageUrl(string url)
    {
        ...
        {
            // BUG FIX: new Uri() lançava UriFormatException para URLs malformadas, quebrando
            // fluxos de download legados. Usa TryCreate para retornar null silenciosamente.
    ...
    private string GetBucket(string path)
    {
        ...
        // BUG FIX: fallback silencioso levava arquivos a buckets errados sem rastro.
        // Agora registramos warning para facilitar auditoria e correção de paths.
        logger.LogWarning("[S3] Path '{Path}' não casou com nenhum padrão de roteamento; usando bucket default ({Bucket}).", path, _config.PrescriptionsBucket);
        return _config.PrescriptionsBucket;
    }

    /// <summary>
    /// Normaliza o path removendo barra inicial e rejeita sequências de traversal/controle.
    /// SECURITY: Sem essa validação, um caller malicioso podia enviar "../outro-bucket-path"
    /// ou caracteres de controle como keys S3 — o S3 aceita qualquer string, mas quando esse
    /// mesmo path fosse reaproveitado em filesystem local (cache/downloads), abria path traversal.
    /// </summary>
    private static string CleanPath(string path)
    {
        ...
        // Rejeita traversal ".." como segmento e barras invertidas.
        if (trimmed.Contains("..", StringComparison.Ordinal) || trimmed.Contains('\\'))
            throw new ArgumentException($"Path inválido (traversal ou backslash): '{path}'", nameof(path));

        // Rejeita caracteres de controle (\0, \r, \n, etc.)
        for (int i = 0; i < trimmed.Length; i++)
        {
            if (char.IsControl(trimmed[i]))
                throw new ArgumentException($"Path contém caractere de controle em posição {i}.", nameof(path));
        }
