...
public sealed class PubMedClient
{
    ...
    {
        // BUG FIX: BuildCochraneQuery/BuildMetaAnalysisQuery retornam "" quando `terms` está vazio.
        // Antes seguíamos para HTTP com term=, gastando cota e recebendo erro. Agora short-circuit.
        ...
        try
        {
            ...
            try
            {
                ...
                // Dispose the first response BEFORE retrying to avoid leaking the socket.
    ...
    // BUG FIX: `.First()` estourava InvalidOperationException quando `terms` vinha
    // vazio (ex.: tradução sem termos MeSH). Agora retornamos query vazia e o caller
    // pode short-circuitar a busca.
    private static string BuildCochraneQuery(List<string> terms)
    {
        if (terms == null || terms.Count == 0) return string.Empty;
        var mainTerm = FormatTerm(terms[0]);
