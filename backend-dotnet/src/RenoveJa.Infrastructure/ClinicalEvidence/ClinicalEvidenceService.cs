...
Inclua apenas artigos relevantes (descarte os irrelevantes). Máximo 5 artigos no resultado.";
        ...
        // LGPD: clinicalContext é construído a partir da anamnese (input de paciente/médico)
        // e pode vazar PII. Redact CPF/RG/telefone/email antes de enviar à OpenAI/Gemini.
        var sanitizedContext = AiReading.PromptSanitizer.RedactPiiOnly(clinicalContext);
...
Analise e retorne JSON com os artigos relevantes que confirmam ou contestam a hipótese.";
        ...
        try
        {
            ...
            try
            {
                response = await client.PostAsync($"{baseUrl}/chat/completions", content, ct);
                ...
                string gptContent = "[]";
                if (responseDoc.RootElement.TryGetProperty("choices", out var choicesEl1)
                    && choicesEl1.ValueKind == JsonValueKind.Array
                    && choicesEl1.GetArrayLength() > 0
                    && choicesEl1[0].TryGetProperty("message", out var msgEl1)
                    && msgEl1.TryGetProperty("content", out var contentEl1))
                {
                    gptContent = contentEl1.GetString() ?? "[]";
                }
...
Example: [""otitis media"", ""ear pain and fever""]";
            ...
            // BUG FIX: GetProperty encadeado lançava KeyNotFoundException em payloads degradados.
            string gptContent = "[]";
            if (responseDoc.RootElement.TryGetProperty("choices", out var choicesEl2)
                && choicesEl2.ValueKind == JsonValueKind.Array
                && choicesEl2.GetArrayLength() > 0
                && choicesEl2[0].TryGetProperty("message", out var msgEl2)
                && msgEl2.TryGetProperty("content", out var contentEl2))
            {
                gptContent = contentEl2.GetString() ?? "[]";
            }
...
{termsJoined}";
            ...
            using var content = new StringContent(jsonBody, Encoding.UTF8, "application/json");
            using var response = await client.PostAsync($"{geminiBaseUrl}/chat/completions", content, ct);
            ...
            // BUG FIX: GetProperty encadeado lançava KeyNotFoundException em payloads degradados.
            string gptContent = "[]";
            if (responseDoc.RootElement.TryGetProperty("choices", out var choicesEl3)
                && choicesEl3.ValueKind == JsonValueKind.Array
                && choicesEl3.GetArrayLength() > 0
                && choicesEl3[0].TryGetProperty("message", out var msgEl3)
                && msgEl3.TryGetProperty("content", out var contentEl3))
            {
                gptContent = contentEl3.GetString() ?? "[]";
            }
