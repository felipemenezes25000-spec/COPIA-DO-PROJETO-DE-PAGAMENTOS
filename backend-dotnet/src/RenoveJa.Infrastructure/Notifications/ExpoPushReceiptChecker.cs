...
public class ExpoPushReceiptChecker : BackgroundService
{
    ...
    // Fila limitada: evita OOM se o background worker parar de drenar (ex.: Expo receipts API caída por horas).
    // Em 300 msgs/batch × 2 batches/hora, 10k tickets cobre ~16 horas de backlog antes de começar a descartar.
    private const int MaxPendingTickets = 10_000;
    ...
    {
        // Bounded: descarta novos receipts se a fila estourou (evita OOM).
        if (_pendingTickets.Count >= MaxPendingTickets)
        {
            _logger.LogWarning("[ExpoReceipts] Fila cheia ({Max}) — descartando ticket {TicketId}", MaxPendingTickets, ticketId);
            return;
        }
    ...
    private async Task CheckReceiptsAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            // BUG FIX: HttpResponseMessage não era descartada — vazava socket por execução do background worker.
            using var response = await client.PostAsJsonAsync(ReceiptsUrl, new { ids = ticketIds }, ct);
            var body = await response.Content.ReadAsStringAsync(ct);

            if (!response.IsSuccessStatusCode)
            {
                // BUG FIX: Antes tentávamos parsear o body como JSON de receipts mesmo quando Expo
                // retornou 429/5xx (body pode ser HTML ou JSON de erro), o que lançava JsonException
                // ou iterava shape errado. Agora recolocamos na fila e saímos cedo.
                _logger.LogWarning("[ExpoReceipts] Expo retornou {Status} — recolocando {Count} tickets na fila",
                    response.StatusCode, entries.Count);
                foreach (var e in entries) _pendingTickets.Enqueue(e);
                return;
            }
