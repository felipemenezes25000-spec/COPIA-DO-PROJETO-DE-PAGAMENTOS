...
public class AuthToken : Entity
{
    ...
    public static AuthToken Reconstitute(
        ...
        DateTime? refreshTokenExpiresAt = null)
    {
        var normalizedExpiresAt = expiresAt.Kind == DateTimeKind.Utc
            ? expiresAt
            : DateTime.SpecifyKind(expiresAt, DateTimeKind.Utc);
        DateTime? normalizedRefreshExpiresAt = refreshTokenExpiresAt.HasValue
            ? (refreshTokenExpiresAt.Value.Kind == DateTimeKind.Utc
                ? refreshTokenExpiresAt.Value
                : DateTime.SpecifyKind(refreshTokenExpiresAt.Value, DateTimeKind.Utc))
            : null;

        return new AuthToken(id, userId, token, normalizedExpiresAt, refreshToken, normalizedRefreshExpiresAt, createdAt);
    }
