...
public class AuditLog : Entity
{
    ...
    private AuditLog(
        ...
        Dictionary<string, object?>? metadata) : base()
    {
        if (string.IsNullOrWhiteSpace(action))
            throw new Exceptions.DomainException("Action is required");
        if (string.IsNullOrWhiteSpace(entityType))
            throw new Exceptions.DomainException("Entity type is required");
