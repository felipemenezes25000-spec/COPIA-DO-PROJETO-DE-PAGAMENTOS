...
public class AuthServiceTests
{
    ...
    public async Task RegisterAsync_ShouldCreateUserAndToken_WhenValidRequest()
    {
        ...
        _userRepositoryMock.Setup(x => x.ExistsByEmailAsync(It.IsAny<string>(), default))
            .ReturnsAsync(false);
        _userRepositoryMock.Setup(x => x.ExistsByCpfAsync(It.IsAny<string>(), It.IsAny<UserRole>(), It.IsAny<Guid?>(), default))
            .ReturnsAsync(false);
    ...
    public async Task RegisterAsync_ShouldThrow_WhenCpfAlreadyExists()
    {
        ...
        _userRepositoryMock.Setup(x => x.ExistsByEmailAsync(It.IsAny<string>(), default))
            .ReturnsAsync(false);
        _userRepositoryMock.Setup(x => x.ExistsByCpfAsync(It.IsAny<string>(), UserRole.Patient, It.IsAny<Guid?>(), default))
            .ReturnsAsync(true);

        Func<Task> act = async () => await _authService.RegisterAsync(request);

        await act.Should().ThrowAsync<AuthConflictException>()
            .WithMessage("*Este CPF já está cadastrado como paciente*");
    }

    [Fact]
    public async Task RegisterAsync_ShouldAllow_WhenCpfExistsAsDoctor()
    {
        // Mesma pessoa pode ter cadastros distintos: paciente novo + médico já existente.
        var request = new RegisterRequestDto(
            "John Doe",
            "john@example.com",
            "password123",
        ...
        _userRepositoryMock.Setup(x => x.ExistsByEmailAsync(It.IsAny<string>(), default))
            .ReturnsAsync(false);
        // Existe como médico, mas NÃO como paciente.
        _userRepositoryMock.Setup(x => x.ExistsByCpfAsync(It.IsAny<string>(), UserRole.Doctor, It.IsAny<Guid?>(), default))
            .ReturnsAsync(true);
        _userRepositoryMock.Setup(x => x.ExistsByCpfAsync(It.IsAny<string>(), UserRole.Patient, It.IsAny<Guid?>(), default))
            .ReturnsAsync(false);
    ...
    [Fact]
    public async Task CompleteProfileAsync_ShouldThrow_WhenCpfDuplicatedInSameRole()
    {
        // Cenário pós-Google: usuário paciente tenta concluir cadastro com CPF de OUTRO paciente.
        var googleUser = User.CreateFromGoogleIdentity("Maria", "maria@gmail.com", BCrypt.Net.BCrypt.HashPassword("x"), UserRole.Patient);

        _userRepositoryMock.Setup(x => x.GetByIdAsync(googleUser.Id, default))
            .ReturnsAsync(googleUser);
        // CPF já existe como OUTRO paciente.
        _userRepositoryMock.Setup(x => x.ExistsByCpfAsync(It.IsAny<string>(), UserRole.Patient, googleUser.Id, default))
            .ReturnsAsync(true);

        var request = new CompleteProfileRequestDto("11987654321", "529.982.247-25", new DateTime(1990, 5, 15));

        Func<Task> act = async () => await _authService.CompleteProfileAsync(googleUser.Id, request);
    ...
    [Fact]
    public async Task CompleteProfileAsync_ShouldNotBlockSelf_WhenUpdatingOwnProfile()
    {
        // exceptUserId garante que o próprio usuário não dispare o conflict.
        var googleUser = User.CreateFromGoogleIdentity("João", "joao@gmail.com", BCrypt.Net.BCrypt.HashPassword("x"), UserRole.Patient);

        _userRepositoryMock.Setup(x => x.GetByIdAsync(googleUser.Id, default))
            .ReturnsAsync(googleUser);
        // Quando exceptUserId == googleUser.Id, retorna false (ele é o único).
        _userRepositoryMock.Setup(x => x.ExistsByCpfAsync(It.IsAny<string>(), UserRole.Patient, googleUser.Id, default))
            .ReturnsAsync(false);
        ...
        var result = await _authService.CompleteProfileAsync(googleUser.Id, request);

        result.Should().NotBeNull();
        _userRepositoryMock.Verify(x => x.UpdateAsync(It.IsAny<User>(), default), Times.AtLeastOnce);
    }
