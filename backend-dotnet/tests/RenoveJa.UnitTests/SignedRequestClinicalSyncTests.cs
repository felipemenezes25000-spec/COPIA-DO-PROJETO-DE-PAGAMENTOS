...
public class SignedRequestClinicalSyncTests
{
    ...
    public async Task SyncSignedRequestAsync_WhenDocumentAlreadyExists_ShouldReturnEarly_Idempotent()
    {
        ...
        // Idempotência: a verificação de existência deve ocorrer exatamente uma vez por chamada.
        medicalDocRepo.Verify(
            r => r.GetBySourceRequestIdAsync(requestId, DocumentType.Prescription, It.IsAny<CancellationToken>()),
            Times.Once);
    }
    ...
    public async Task SyncSignedRequestAsync_WhenRequestIsExam_ShouldCreateExamOrderAndDocument()
    {
        ...
        // signedAt deve ser >= Encounter.StartedAt (que é DateTime.UtcNow capturado dentro de
        // Encounter.Start). Usar `now` direto causa race: o StartedAt fica microssegundos depois,
        // e FinalizeEncounter dispara "FinishedAt cannot be before StartedAt" — exceção silenciosamente
        // engolida pelo catch do serviço, o que faz a verificação de CreateAsync falhar.
        await sut.SyncSignedRequestAsync(
            request,
            "https://storage/signed.pdf",
            "sig-123",
