...
// SECURITY: em produção, NÃO permitir fallback para localhost — erro alto e visível.
if (!RAW_API_URL && !__DEV__) {
  throw new Error(
    '[ApiClient] EXPO_PUBLIC_API_URL não definido em build de produção. ' +
    'Configure a URL da API antes de gerar o build (eas.json / app.config).',
  );
}
...
class ApiClient {
  ...
  /** True quando já consultamos o SecureStore pelo menos uma vez (distingue "vazio" de "não-carregado"). */
  private tokenLoaded = false;
  /** Promise compartilhada para evitar múltiplas leituras concorrentes do SecureStore. */
  ...
  clearTokenCache() {
    this.tokenCache = null;
    // tokenLoaded permanece true: sabemos que o token foi limpo, não precisa reler do SecureStore.
    this.tokenLoaded = true;
  }
  ...
  private async getAuthHeader(): Promise<Record<string, string>> {
    ...
    // Concurrent callers compartilham a mesma Promise (evita N leituras simultâneas do SecureStore).
    if (!this.tokenLoadingPromise) {
      this.tokenLoadingPromise = getSecureItem(AUTH_TOKEN_KEY)
        .then((token) => {
    ...
    const token = await this.tokenLoadingPromise;
  ...
  private async fetchWithTimeout(url: string, init: RequestInit, timeoutMs: number = REQUEST_TIMEOUT_MS): Promise<Response> {
    ...
    try {
      ...
      if (err?.name === 'AbortError') {
        ...
        // Lança Error real para que `instanceof Error` e stack trace funcionem corretamente
  ...
  private async handleResponse<T>(
    ...
    if (!response.ok) {
      ...
      try {
        ...
        if (rawBody) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any -- ApiError shape varia por endpoint
          let errorData: any = {};
          try {
            errorData = JSON.parse(rawBody);
          } catch {
            // Backend retornou texto não-JSON (ex: HTML de proxy/erro 502 Cloudfront).
            // Manter errorMessage default e seguir.
            if (__DEV__) console.warn('[api-client] Resposta de erro não é JSON válido:', rawBody.slice(0, 200));
          }
          ...
          if (response.status === 403) {
            ...
            if (this.onForbidden) this.onForbidden(errorMessage);
          }
  ...
  async getAuthToken(): Promise<string | null> {
    if (this.tokenLoaded) return this.tokenCache;
    if (!this.tokenLoadingPromise) {
      this.tokenLoadingPromise = getSecureItem(AUTH_TOKEN_KEY)
        .then((token) => {
          this.tokenCache = token;
