...
export interface RequestClaimedPayload {
  requestId: string;
  claimedByDoctorName: string;
  claimedAt: string;
}

export interface RequestReleasedPayload {
  requestId: string;
}

export type RequestClaimedListener = (payload: RequestClaimedPayload) => void;
export type RequestReleasedListener = (payload: RequestReleasedPayload) => void;

let connection: any = null;
let connectingPromise: Promise<boolean> | null = null;
const listeners = new Set<RequestUpdatedListener>();
const claimedListeners = new Set<RequestClaimedListener>();
const releasedListeners = new Set<RequestReleasedListener>();
...
export async function startRequestsEventsConnection(): Promise<boolean> {
  ...
  const doConnect = async (): Promise<boolean> => {
    ...
    try {
      ...
      conn.on('RequestClaimed', (payload: RequestClaimedPayload) => {
        claimedListeners.forEach((fn) => {
          try {
            fn(payload);
          } catch (e) {
            if (__DEV__) console.warn('[RequestsEvents] RequestClaimed listener error:', e);
          }
        });
      });

      conn.on('RequestReleased', (payload: RequestReleasedPayload) => {
        releasedListeners.forEach((fn) => {
          try {
            fn(payload);
          } catch (e) {
            if (__DEV__) console.warn('[RequestsEvents] RequestReleased listener error:', e);
          }
        });
      });
...
/**
 * Inscreve um listener para o evento RequestClaimed.
 * Retorna função para cancelar a inscrição.
 */
export function subscribeRequestClaimed(listener: RequestClaimedListener): () => void {
  claimedListeners.add(listener);
  return () => claimedListeners.delete(listener);
}

/**
 * Inscreve um listener para o evento RequestReleased.
 * Retorna função para cancelar a inscrição.
 */
export function subscribeRequestReleased(listener: RequestReleasedListener): () => void {
