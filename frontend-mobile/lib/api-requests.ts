...
/**
 * Lista pedidos rejeitados automaticamente pela IA para o médico autenticado
 * (filtrados por especialidade no backend). Endpoint: GET /api/requests/ai-rejected.
 */
export async function fetchAiRejectedRequests(
  options?: { signal?: AbortSignal }
): Promise<RequestResponseDto[]> {
  const res = await apiClient.get<RequestResponseDto[] | { items?: RequestResponseDto[] }>(
    '/api/requests/ai-rejected',
    undefined,
    options
  );
...
export async function reopenAiRejection(requestId: string): Promise<RequestResponseDto> {
  return apiClient.post(`/api/requests/${requestId}/reopen-ai-rejection`, {});
}
...
/**
 * Claim explícito do pedido ('Iniciar revisão'). Retorna o pedido atualizado.
 * Lança erro com status 409 se outro médico já pegou — o body do erro contém
 * `claimedBy` com o nome do médico atual.
 */
export async function claimRequest(id: string): Promise<{ request: RequestResponseDto }> {
  return apiClient.post(`/api/requests/${id}/claim`, {});
}
