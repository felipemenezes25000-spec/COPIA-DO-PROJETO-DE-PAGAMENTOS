...
describe('getRequestUiState', () => {
  ...
  it('approved + prescription retorna "Aguardando assinatura" (não "Aprovado")', () => {
    // Regressão: o label "Aprovado" era ambíguo — médico interpretava como
    // finalizado, mas na verdade o documento ainda aguarda assinatura digital.
    // Alinhado ao StatusTracker, que usa esta label na mesma etapa.
    const res = mapUi(req('approved', { requestType: 'prescription' }));
    expect(res.uiState).toBe('needs_action');
    expect(res.label).toBe('Aguardando assinatura');
    expect(res.colorKey).toBe('waiting');
  });

  it('approved + exam retorna "Aguardando assinatura"', () => {
    const res = mapUi(req('approved', { requestType: 'exam' }));
    expect(res.label).toBe('Aguardando assinatura');
    expect(res.colorKey).toBe('waiting');
  });

  it('approved + consultation NÃO retorna "Aguardando assinatura"', () => {
    // Consulta no estado "approved" significa "pronta para vídeo", não
    // "aguardando assinatura" — lógica distinta no getRequestUiState.
    const res = mapUi(req('approved', { requestType: 'consultation' }));
    expect(res.label).toBe('Consulta pronta');
    expect(res.uiState).toBe('ready');
  });
});
