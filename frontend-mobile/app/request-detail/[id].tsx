...
function isUnclaimedForDoctor(request: { doctorId: string | null }, userRole: string): boolean {
...
export default function RequestDetailScreen() {
  ...
  useRequireAuth(undefined, { requireProfileComplete: true });
  const { user } = useAuth();
  ...
  // Redirect doctors to the editor only when the request is already claimed
  // (has a doctorId). Unclaimed requests stay here so the doctor can claim first.
  useEffect(() => {
    if (user?.role === 'doctor' && requestId && request && request.doctorId) {
      router.replace(`/doctor-request/${requestId}`);
    }
  }, [user?.role, requestId, request, router]);
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      if (err?.status === 409) {
        ...
        showToast({ message: `${holder} já pegou este pedido.`, type: 'warning' });
        await queryClient.invalidateQueries({ queryKey: DOCTOR_REQUESTS_QUERY_KEY });
        router.back();
      } else {
        showToast({ message: (err as Error)?.message || 'Não foi possível iniciar a revisão.', type: 'error' });
      }
    } finally {
      setIsClaiming(false);
    }
    ...
    <SafeAreaView style={styles.container} edges={['bottom', 'left', 'right']}>
      ...
      <FadeIn visible={!!request} {...motionTokens.fade.patient}>
        ...
        >
          ...
          {/* ─── Claim banner (doctor, unclaimed) OR Cancel Button (patient) ─── */}
          {user && request && isUnclaimedForDoctor(request, user.role) ? (
            <View style={styles.claimContainer}>
              ...
              <TouchableOpacity
                ...
                disabled={isClaiming}
                accessibilityRole="button"
                accessibilityLabel="Iniciar revisão"
              >
                {isClaiming ? (
