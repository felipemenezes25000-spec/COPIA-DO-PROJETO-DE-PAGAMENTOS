...
// IMPORTANTE: react-native-webview entrega 'ref.postMessage(...)' em
// \`document\` no Android e em \`window\` no iOS. Precisamos escutar AMBOS
// senão o PDF não renderiza no Android (tela branca / "Aguardando dados...").
function onPdfMessage(e) {
  // Ignora mensagens que não parecem base64 de PDF (ex.: hot reload do Metro).
  var data = e && e.data;
...
document.addEventListener('message', onPdfMessage);
<\/script>
</body></html>`;
}
                ...
                {Platform.OS === 'web' ? (
                  ...
                  <WebView
                    ...
                    onLoad={() => {
                      ...
                      // IMPORTANTE: react-native-webview entrega em `document` no Android e
                      // em `window` no iOS — o listener no HTML escuta nos dois alvos.
                    ...
                    onLoadEnd={() => {
                      // Reenvio defensivo: em Android o CDN do pdf.js pode carregar
                      // após o onLoad disparar — reemitimos depois que TODOS os assets
                      // terminaram, garantindo que o listener já está ativo.
                      if (pdfUri && webViewRef.current) {
                        const base64 = pdfUri.replace(/^data:application\/pdf;base64,/, '');
                        webViewRef.current.postMessage(base64);
                      }
