...
function getEncounterMeta(colors: DesignColors): Record<string, { icon: keyof typeof Ionicons.glyphMap; color: string; bg: string; label: string }> {
  const teleconsulta = { icon: 'videocam' as const, color: colors.info, bg: colors.infoLight, label: 'Teleconsulta' };
  const renovacao = { icon: 'refresh' as const, color: colors.success, bg: colors.successLight, label: 'Renovação de receita' };
  const exame = { icon: 'flask' as const, color: colors.accent, bg: colors.accentSoft, label: 'Exame' };
  const receita = { icon: 'medical' as const, color: colors.accent, bg: colors.accentSoft, label: 'Receita' };
  return {
    // PT-BR
    teleconsulta,
    ...
    receita,
    exame,
    // EN / enum names (PascalCase vindo do backend, já normalizado para lowercase)
    consultation: teleconsulta,
    teleconsultation: teleconsulta,
    prescription: receita,
    prescriptionrenewal: renovacao,
    renewal: renovacao,
    exam: exame,
    examorder: exame,
    exam_order: exame,
