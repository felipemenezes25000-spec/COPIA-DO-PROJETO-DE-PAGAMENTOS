  ...
  useEffect(() => {
    ...
    if (!loading) {
      const navTimer = setTimeout(async () => {
        if (hasNavigated.current) return;
        if (user) {
          // Calcula o destino "real" pós-login
          let target: string;
          if (user.role === 'patient') {
            // Pacientes vindos de login Google podem ter cadastro incompleto:
            // forçamos a tela de complete-profile até que terminem o onboarding.
            target = !user.profileComplete ? '/(auth)/complete-profile' : '/(patient)/home';
          } else if (user.role === 'doctor' && !user.profileComplete) {
            target = '/(auth)/complete-doctor';
          } else {
            target = '/(doctor)/dashboard';
          }

          // Gate de permissões obrigatórias — câmera, microfone, notificações.
          // Só vale para usuários autenticados; pré-login segue fluxo normal.
          // A tela /permissions redireciona para `next` quando todas forem concedidas.
          const ok = await allRequiredGranted();
          if (!ok) {
            navigate(`/permissions?next=${encodeURIComponent(target)}`);
          } else {
            navigate(target);
          }
        } else {
