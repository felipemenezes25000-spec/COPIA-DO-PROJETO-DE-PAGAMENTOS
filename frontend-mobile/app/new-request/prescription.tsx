...
export default function NewPrescription() {
  ...
  const pickFromGallery = async () => {
    ...
    if (remaining <= 0) {
      Alert.alert('Limite atingido', 'Máximo de 5 arquivos por renovação.');
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    if (isConnected === false) {
      Alert.alert('Sem conexão', 'Conecte-se à internet para enviar sua renovação.');
      return;
    }
    ...
    try {
      ...
      invalidateRequests();
      showToast({ message: 'Renovação enviada! Acompanhe na aba Pedidos.', type: 'success' });
        ...
        >
                ...
                return (
                  <View key={`${uri}-${index}`} style={styles.imageContainer}>
                    ...
                    <TouchableOpacity
                      ...
                      accessibilityRole="button"
                      accessibilityLabel={`Remover ${isPdf ? 'arquivo' : 'foto'} ${index + 1}`}
                    >
          ...
          <View style={styles.infoBox}>
            ...
            <Text style={styles.infoText}>
              Sua renovação será analisada por um médico em até 15 minutos.
            </Text>
          </View>
        </ScrollView>
