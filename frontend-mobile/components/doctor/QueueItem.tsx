...
// ── Main component ──
export type QueueItemStatusTone =
  | 'approved'   // verde — aprovado pelo médico, aguardando assinatura em lote
  | 'rejected'   // vermelho — rejeitado definitivamente
  | 'signed';    // roxo — assinado/entregue
...
interface QueueItemProps {
  ...
  colors: DesignColors;
  /**
   * Override opcional do badge de status para casos em que o contexto visual
   * (ex.: pasta de estado na fila do médico) diverge do uiState canônico
   * retornado por getRequestUiState. Passando, por exemplo, 'approved' aqui
   * renderiza um chip verde "Aprovado" mesmo que getRequestUiState classifique
   * o request como 'needs_action' (que é o default para a visão do paciente).
   */
  statusTone?: QueueItemStatusTone;
}

const TONE_STYLES: Record<QueueItemStatusTone, { bg: string; text: string; label: string }> = {
  approved: { bg: '#D1FAE5', text: '#047857', label: 'Aprovado' },   // emerald-100 / emerald-700
  rejected: { bg: '#FEE2E2', text: '#B91C1C', label: 'Rejeitado' },  // red-100 / red-700
  signed:   { bg: '#EDE9FE', text: '#6D28D9', label: 'Assinado' },   // violet-100 / violet-700
};

export const QueueItem = React.memo(function QueueItem({
  request, onPress, colors, statusTone,
}: QueueItemProps) {
  const { label: defaultLabel, colorKey, uiState } = getRequestUiState(request);
  const toneStyle = statusTone ? TONE_STYLES[statusTone] : null;
  const label = toneStyle?.label ?? defaultLabel;
  ...
  const baseStatusStyle = getStatusStyle(colorKey, uiState);
  // Quando há override de tom (pasta de estado no dashboard do médico), sobrepõe
  // as cores do chip mas preserva o `hasPulse`/`borderColor` do estilo base (ex.:
  // in_consultation mantém a borda verde pulsante independentemente do tom).
  const statusStyle = toneStyle
    ? { ...baseStatusStyle, bg: toneStyle.bg, text: toneStyle.text }
    : baseStatusStyle;
