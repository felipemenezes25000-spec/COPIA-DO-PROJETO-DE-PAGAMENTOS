...
function createTimeoutRace(ms: number) {
  ...
  const promise = new Promise<never>((_, reject) => {
    id = setTimeout(() => {
      // Se clear() já rodou (a outra promise venceu o race), não rejeita —
      // evita "Possible Unhandled Promise Rejection" porque ninguém mais
      // está com .catch() neste promise depois que o Promise.race resolveu.
      if (cleared) return;
      reject(new Error(INIT_TIMEOUT_MESSAGE));
    }, ms);
  });
  // Anexa um catch no-op para garantir que, se o reject acontecer entre o
  // resolve da outra promise e a próxima microtask, o RN não printe warning.
  promise.catch(() => {});
  return {
    promise,
    clear: () => {
...
export default function VideoCallScreenInner() {
  ...
  useEffect(() => {
    ...
    // FIX: reset fallback flag so a rejoin attempts Deepgram again instead of
    // sticking to Whisper fallback from a previous session on the same component instance.
    setUseFallbackTranscription(false);
  }, [rid, setCallSeconds]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Permissões já foram exigidas pelo gate /permissions (useRequirePermissions).
