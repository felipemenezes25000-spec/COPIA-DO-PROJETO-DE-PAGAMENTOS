...
function buildReferralFromAnamnesis(anamnesis: AnamnesisData | null): { professional: string; specialty?: string; reason: string } | null {
  ...
  const specialty = enc.especialidade ?? '';
  const reason = enc.motivo ?? enc.reason ?? enc.indication ?? '';
  // Retorna se qualquer campo útil foi encontrado
  if (!professional && !reason && !specialty) return null;
  return {
    professional,
...
export default function PostConsultationScreen({ request, onComplete, onBack }: Props) {
  ...
  React.useEffect(() => {
    if (refSug) {
      if (refSug.professional) setRefProfessional(refSug.professional);
      if (refSug.specialty) setRefSpecialty(refSug.specialty);
      if (refSug.reason) setRefReason(refSug.reason);
      // Ativar painel de encaminhamento se a IA sugeriu qualquer dado
      if (refSug.professional || refSug.specialty || refSug.reason) {
        setRefEnabled(true);
        setRefOpen(true);
      }
    }
  ...
  // FIX M3: zera a senha do PFX no unmount e quando o modal de senha fecha.
  // Sem isso, a senha ficava viva em `certPasswordRef.current` até o
  // componente ser destruído — e crash reporters (Sentry/Crashlytics) podiam
  // fazer snapshot de state/refs em caso de OOM, vazando a credencial.
  useEffect(() => {
    if (!passwordModalVisible) {
      certPasswordRef.current = '';
      setCertPassword('');
  ...
  const handleSignClick = () => {
    ...
    // Validação: encaminhamento ativado precisa de campos obrigatórios
    if (refEnabled) {
      if (!refReason.trim() && !refSpecialty.trim()) {
        Alert.alert(
          'Encaminhamento incompleto',
          'Preencha pelo menos a Especialidade e o Motivo/Indicação para emitir o encaminhamento.',
          [{ text: 'Preencher', onPress: () => { setRefOpen(true); } }]
        );
        return;
      }
      if (!refReason.trim()) {
        Alert.alert(
          'Encaminhamento incompleto',
          'O campo Motivo/Indicação é obrigatório para o encaminhamento.',
          [{ text: 'Preencher', onPress: () => { setRefOpen(true); } }]
        );
        return;
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (refEnabled) {
        const reasonText = refReason.trim() || `Encaminhamento para ${refSpecialty.trim() || 'avaliação presencial'} conforme avaliação clínica.`;
        payload.referral = {
          professionalName: refProfessional.trim() || refSpecialty.trim() || 'Especialista',
          ...
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={S.modalKav}>
            <Pressable style={S.modalCard} onPress={(e) => e.stopPropagation()}>
              ...
              <View style={{ position: 'relative', justifyContent: 'center' }}>
                ...
                />
                <TouchableOpacity
                  onPress={() => setShowCertPassword((v) => !v)}
                  style={{ position: 'absolute', right: 10, top: 0, bottom: 0, width: 36, alignItems: 'center', justifyContent: 'center' }}
                  accessibilityRole="button"
                  accessibilityLabel={showCertPassword ? 'Ocultar senha' : 'Mostrar senha'}
                  hitSlop={8}
                  activeOpacity={0.7}
                >
                  <Ionicons
                    name={showCertPassword ? 'eye-off' : 'eye'}
                    size={20}
