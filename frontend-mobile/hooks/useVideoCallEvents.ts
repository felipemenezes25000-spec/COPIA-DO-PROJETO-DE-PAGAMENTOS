...
export function useVideoCallEvents(
      ...
      if (isDoctor) {
        ...
        conn.on('AnamnesisUpdate', (data: Record<string, unknown>) => {
          ...
          try {
            ...
            if (parsed && typeof parsed === 'object') {
              setAnamnesis(parsed as Record<string, unknown>);
            }
          } catch (err) {
            // Antes, o catch era silencioso e o médico via a anamnese antiga sem perceber
            // que o backend enviou JSON inválido — bug de confiança clínica.
            if (__DEV__) console.warn('[useVideoCallEvents] AnamnesisUpdate: JSON inválido', err);
          }
        });
        ...
        conn.on('SuggestionUpdate', (data: Record<string, unknown>) => {
          ...
          if (Array.isArray(items)) {
            // Coerção/filtragem: sem isso, um item não-string do backend quebraria o render
            // da lista de sugestões clínicas e poderia mostrar "[object Object]".
            setSuggestions(items.map((s) => String(s ?? '')).filter(Boolean));
          }
        });
  ...
  // Single cleanup path: the reset effect on [requestId] above already disposes
  // the connection when rid changes or on unmount, so no second effect is needed.
  // Keeping a redundant effect caused two stop() calls in some remount sequences.
