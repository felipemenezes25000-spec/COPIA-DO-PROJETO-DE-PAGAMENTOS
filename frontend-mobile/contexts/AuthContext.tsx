...
/**
 * Limpa a sessão cacheada do Google SDK no dispositivo.
 * `revokeAccess` invalida o consent (força o seletor de contas no próximo login),
 * `signOut` limpa a credencial local. Os dois juntos garantem que o usuário possa
 * trocar de conta após logoff. Falha silenciosa para usuários que não entraram via Google.
 */
async function clearGoogleSession(): Promise<void> {
  try {
    await GoogleSignin.revokeAccess();
  } catch (e) {
    if (__DEV__) console.warn('[Auth] GoogleSignin.revokeAccess falhou:', e);
  }
  try {
    await GoogleSignin.signOut();
  } catch (e) {
    if (__DEV__) console.warn('[Auth] GoogleSignin.signOut falhou:', e);
  }
}
...
interface AuthContextType {
  ...
  updateProfile: (data: UpdateProfileData) => Promise<UserDto>;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // Loading guard único: 3s força liberação da tela. Limpo em sucesso e no unmount.
  ...
  const loadStoredUser = async () => {
    // Loading guard é gerenciado pelo useEffect (loadingGuardRef, 3000ms)
    const clearGuard = () => {
      if (loadingGuardRef.current) {
        clearTimeout(loadingGuardRef.current);
        loadingGuardRef.current = null;
      ...
      if (storedToken && storedUser) {
        ...
        apiClient.get<UserDto>('/api/auth/me', undefined, { signal: controller.signal })
          .then(async (currentUser) => {
            ...
            // Se o ID mudou (ex.: token reciclado), limpa cache para não vazar dados
  ...
  const signIn = useCallback(async (email: string, password: string): Promise<UserDto> => {
    try {
      ...
      // Limpa cache de dados da sessão anterior para evitar vazamento entre usuários
      queryClient.clear();
  ...
  const signOut = useCallback(async () => {
    ...
    // Recicla credencial do Google ANTES de limpar o estado local: garante que o
    // próximo "Continuar com Google" exiba o seletor de contas em vez de auto-logar.
    ...
    // Limpa todo o cache do React Query para não vazar dados entre sessões
    queryClient.clear();
  }, [clearAuth]);
  ...
  const updateProfile = useCallback(async (data: UpdateProfileData): Promise<UserDto> => {
    try {
      const updatedUser = await apiClient.patch<UserDto>('/api/auth/update-profile', data);
      await setItemSafe(USER_KEY, updatedUser ? JSON.stringify(updatedUser) : undefined);
      setUser(updatedUser);
      return updatedUser;
    } catch (error: unknown) {
  ...
  const value = useMemo(() => ({
    ...
    forgotPassword,
    resetPassword,
  }), [user, doctorProfile, loading, signIn, signUp, signUpDoctor, signInWithGoogle, signOut, cancelRegistration, refreshUser, refreshDoctorProfile, completeProfile, updateProfile, forgotPassword, resetPassword]);
