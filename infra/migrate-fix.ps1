...
Write-Host "=== Fase 1: Fix colunas v2 ===" -ForegroundColor Cyan
docker run --rm -v "C:\Users\renat\source\repos\ola-jamal\infra\fix_columns_v2.sql:/fix.sql" -e PGPASSWORD="RnvJa2026#Pr0d!Sx9kL2m" postgres:15 psql -h renoveja-postgres.c54og6486w6w.sa-east-1.rds.amazonaws.com -U postgres -d renoveja -f /fix.sql

Write-Host ""
Write-Host "=== Fase 2: Migrar tabelas que falharam ===" -ForegroundColor Cyan
...
$tables = @("requests","payments","audit_logs","prescriptions","encounters","audit_events","ai_interaction_logs")
