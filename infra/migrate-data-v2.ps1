...
foreach ($t in $tables) {
    try {
        ...
        if ($errLines) {
            ...
            $errLines | Select-Object -First 2 | ForEach-Object { Write-Host "    $_" -ForegroundColor DarkYellow }
        } else {
        ...
        Write-Host "  $t : SKIP ($($_.Exception.Message.Substring(0, [Math]::Min(80, $_.Exception.Message.Length))))" -ForegroundColor Gray
    }
}

Write-Host ""
Write-Host "=== Total migrado: $total rows ===" -ForegroundColor Green

# Verificar
Write-Host ""
Write-Host "Verificando dados no RDS..." -ForegroundColor Cyan
$check = & docker run --rm -e PGPASSWORD="RnvJa2026#Pr0d!Sx9kL2m" postgres:15 psql -h renoveja-postgres.c54og6486w6w.sa-east-1.rds.amazonaws.com -U postgres -d renoveja -c "SELECT 'users: ' || COUNT(*) FROM users UNION ALL SELECT 'requests: ' || COUNT(*) FROM requests UNION ALL SELECT 'payments: ' || COUNT(*) FROM payments UNION ALL SELECT 'doctors: ' || COUNT(*) FROM doctor_profiles ORDER BY 1;" 2>&1
