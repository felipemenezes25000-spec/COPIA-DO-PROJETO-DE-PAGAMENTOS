...
if (-not $BackendOnly) {
    ...
    if (-not (Test-Path $frontendDir)) {
        ...
        Write-Host "  Sync para S3 (dry-run para revisar deletes)..."
        aws s3 sync dist/ "s3://$S3_FRONTEND_BUCKET" --delete --dryrun --region $Region
        $confirm = Read-Host "Confirm DELETE operations above (type YES)"
        if ($confirm -ne "YES") { Write-Host "Aborted." -ForegroundColor Yellow; Pop-Location; exit 1 }
