...
export function useRequestEvents(onEvent?: EventHandler) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!sr || cancelled || abortController.signal.aborted) return;
    ...
    (async () => {
        ...
        (data: { anamnesisJson?: string; AnamnesisJson?: string }) => {
          ...
          setAnamnesis((prev) => {
            ...
            try {
              ...
              // JSON inválido — mantém anamnese anterior em vez de contaminar o estado
              // com string crua que renderizaria como [object String] no painel clínico.
              if (import.meta.env.DEV)
                console.warn(
                  '[useSignalR] AnamnesisUpdate: JSON inválido, mantendo anamnese anterior'
                );
              return prev;
            }
          });
      ...
      connection.on(
        ...
        (data: {
          ...
          // Coerção: itens não-string do backend quebrariam o render como [object Object]
          setSuggestions(
            Array.isArray(items)
