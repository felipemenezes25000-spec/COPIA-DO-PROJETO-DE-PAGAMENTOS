...
export function useTheme() {
  ...
  useEffect(() => {
    ...
    // Track every timeout the handler creates so rapid theme changes
    // never leak timers (antes, só o último era limpo e os anteriores vazavam).
    const timers = new Set<number>();
    const handler = () => {
      const id = applyTheme('system');
      timers.add(id);
      // Remove do set após o timeout resolver naturalmente
      window.setTimeout(() => timers.delete(id), 350);
    };
