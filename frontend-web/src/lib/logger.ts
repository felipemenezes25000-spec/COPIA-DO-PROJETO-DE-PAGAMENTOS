...
// Em produção, trace/debug/info não vão ao console — evita poluir devtools de usuários
// finais e vazar dados sensíveis acidentalmente via logs verbosos.
...
function log(
  ...
  if (IS_PROD && (level === 'trace' || level === 'debug')) return;
  ...
  const args = [
    prefix,
    message,
    ...(Object.keys(safeAttrs).length ? [safeAttrs] : []),
  ];
