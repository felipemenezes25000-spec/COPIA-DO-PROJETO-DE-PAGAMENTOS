...
export default function DoctorVideoCall() {
  ...
  // FIX: When the server signals ConsultationEnded (e.g. server-side timeout,
  // or mobile doctor finishing from the other device), the web call UI must
  // react — otherwise the doctor sees a frozen call with the room already closed.
  // Route /resumo-consulta/:requestId é a rota real (ver DoctorApp.tsx:249) e
  // é a mesma usada por handleCallLeft/handleFinish — consistência entre as 3
  // saídas da videoconsulta.
  useEffect(() => {
    if (!consultationEnded || !requestId) return;
    toast.info('Consulta encerrada pelo servidor.');
    if (timerRef.current) {
      clearInterval(timerRef.current);
      timerRef.current = null;
    }
    navigate(`/resumo-consulta/${requestId}`);
  }, [consultationEnded, requestId, navigate]);
  ...
  useEffect(() => {
    if (consultationStarted) {
      // Clear any prior interval before creating a new one
      if (timerRef.current) {
        clearInterval(timerRef.current);
        timerRef.current = null;
      }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (data.patientId) {
          try {
            const profile = await getPatientProfile(data.patientId);
            if (cancelled || signal.aborted) return;
            setPatient(profile);
          } catch {
            /* paciente opcional */
        ...
        if (needsStart && !startingRef.current) {
          ...
          try {
            ...
            // FIX: startConsultation retorna { request: { consultationStartedAt, ... } }.
            // Se o backend já tinha a consulta rodando (idempotente), o timer deve
            // iniciar do elapsed real — não de zero — para não mentir sobre o tempo
            // decorrido quando o médico recarrega a página no meio da consulta.
            const startedAt: string | undefined =
              result?.request?.consultationStartedAt ??
              result?.consultationStartedAt;
            if (startedAt) {
