      ...
      {files.map((f) => {
        ...
        return (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex items-center gap-2 rounded-lg border border-border/50 bg-muted/50 p-2 text-sm transition-colors hover:bg-muted"
          >
            {isImage ? (
...
export default function DoctorCarePlan() {
  ...
  const handleTaskAction = async (
    taskId: string,
    action: 'start' | 'complete' | 'submit_results'
  ) => {
    if (!carePlan) return;
  ...
  const inProgressTasks = carePlan.tasks.filter(
    (t) => t.state === 'in_progress'
  );
  const submittedTasks = carePlan.tasks.filter((t) => t.state === 'submitted');
  const closedTasks = carePlan.tasks.filter((t) =>
    ['reviewed', 'closed', 'completed'].includes(t.state)
  );
      ...
      <div className="max-w-2xl space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <div className="mt-0.5 flex items-center gap-2">
              <Badge className={getStatusBadge(carePlan.status)}>
                {formatStatus(carePlan.status)}
              </Badge>
              <span className="text-xs text-muted-foreground">
                {carePlan.tasks.length} tarefa
                {carePlan.tasks.length !== 1 ? 's' : ''}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          {carePlan.tasks.map((task: CarePlanTask, i: number) => {
            ...
            const isDone = ['reviewed', 'closed', 'completed'].includes(
              task.state
            );
            ...
            return (
              ...
              >
                <Card
                  className={`shadow-sm ${isSubmitted ? 'border-blue-200 dark:border-blue-800' : isDone ? 'border-emerald-200 opacity-75 dark:border-emerald-800' : ''}`}
                >
                  <CardHeader className="pb-2">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge
                        className={`text-[10px] ${getStatusBadge(task.state)}`}
                      >
