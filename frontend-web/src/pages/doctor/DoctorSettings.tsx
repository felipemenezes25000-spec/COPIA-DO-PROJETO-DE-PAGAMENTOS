...
export default function DoctorSettings() {
  ...
  useEffect(() => {
    let cancelled = false;
    // getMutedKeys lê de localStorage — falhas são raras (storage corrompido).
    // Um silent catch aqui é aceitável mas logamos para debugging.
    getMutedKeys()
      .then((keys) => {
        if (!cancelled) setMutedCount(keys.length);
      })
      .catch((err) => {
        console.warn('getMutedKeys falhou:', err);
      });
    // getPushPreferences é API remota — se falhar, o médico veria defaults
    // sem saber que a carga falhou e pensaria que já configurou algo.
    getPushPreferences()
      .then((prefs) => {
      ...
      <div className="max-w-2xl space-y-6">
          ...
          <CardContent className="space-y-2 pt-0">
            <div className="flex items-center justify-between">
              <span className="text-sm">
                Reativar mensagens silenciadas ({mutedCount})
              </span>
              {mutedCount > 0 ? (
        ...
        <Card className="shadow-sm">
          ...
          <CardContent className="space-y-2">
            ...
            >
              <Link to="/sobre">
                <Info className="h-4 w-4" /> Sobre o RenoveJá+
              </Link>
            </Button>
            <Button
              variant="ghost"
            ...
            >
              <Link to="/ajuda">
                <HelpCircle className="h-4 w-4" /> Ajuda e FAQ
              </Link>
            </Button>
            <Button
              variant="ghost"
