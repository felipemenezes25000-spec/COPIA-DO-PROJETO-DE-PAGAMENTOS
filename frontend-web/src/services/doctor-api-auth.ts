...
// FIX M2: o mutex original limpava `refreshPromise` no `finally` DEPOIS de
// awaitar — havia uma janela microscópica entre a resolução da promise e a
// limpeza do ref em que um novo 401 podia disparar um SEGUNDO refresh.
// Na prática raramente gerava dois pares de tokens, mas com rede
// "flakeada" e múltiplos fetches simultâneos a janela expandia.
// Agora gravamos `refreshPromise` atomicamente e só limpamos APÓS o microtask
// de resolução ter propagado — qualquer 401 que entrou durante a execução
...
export async function tryRefreshToken(): Promise<boolean> {
  ...
  const p = executeRefresh().finally(() => {
    // Limpa somente depois que a promise acabou de resolver — garante que
    // qualquer chamada concorrente que chegou enquanto `executeRefresh`
    // estava em andamento pegou a MESMA promise via `refreshPromise`.
...
const AUTH_ENDPOINT_RE =
  /\/api\/auth\/(login|register|refresh|google|forgot-password|reset-password)/;
...
export async function registerDoctorFull(payload: {
  ...
  crmState: string;
  specialty: string;
  // Dados pessoais (birthDate obrigatório pelo backend)
  birthDate: string; // ISO YYYY-MM-DD
  gender?: 'M' | 'F' | 'Outro' | 'Não informado';
  // Endereço (street, number, neighborhood, city, state obrigatórios)
  street: string;
  number: string;
  neighborhood: string;
  complement?: string;
  ...
  const body = {
    ...payload,
    // O backend espera BirthDate como DateTime — ISO string serializa bem
