...
function normalizeList(raw: unknown): unknown {
  ...
  if (obj && typeof obj === 'object') {
    const arr = (obj.items ?? obj.data) as
      | Record<string, unknown>[]
      | undefined;
...
export async function acceptConsultation(id: string) {
  const res = await authFetch(`/api/requests/${id}/accept-consultation`, {
    method: 'POST',
  });
...
export async function reanalyzePrescription(id: string) {
  const res = await authFetch(`/api/requests/${id}/reanalyze-prescription`, {
    method: 'POST',
  });
...
export async function reanalyzeExam(id: string) {
  const res = await authFetch(`/api/requests/${id}/reanalyze-exam`, {
    method: 'POST',
  });
...
export async function reanalyzeAsDoctor(id: string) {
  const res = await authFetch(`/api/requests/${id}/reanalyze-as-doctor`, {
    method: 'POST',
  });
...
export async function markRequestDelivered(id: string) {
  const res = await authFetch(`/api/requests/${id}/mark-delivered`, {
    method: 'POST',
  });
...
export async function generatePdf(
  ...
  const res = await authFetch(`/api/requests/${id}/generate-pdf`, {
    method: 'POST',
  });
...
export async function getDocumentDownloadUrl(id: string): Promise<string> {
  ...
  const res = await authFetch(`/api/requests/${id}/document-token`, {
    method: 'POST',
  });
...
export async function assignToQueue(id: string) {
  const res = await authFetch(`/api/requests/${id}/assign-queue`, {
    method: 'POST',
  });
...
export async function claimRequest(
  ...
  const res = await authFetch(`/api/requests/${id}/claim`, { method: 'POST' });
  if (!res.ok) {
    const body = await res.json().catch(() => ({}));
    throw Object.assign(
      new Error((body as { error?: string }).error ?? 'Claim falhou'),
...
export async function reopenAiRejection(id: string): Promise<MedicalRequest> {
  const res = await authFetch(`/api/requests/${id}/reopen-ai-rejection`, {
    method: 'POST',
  });
