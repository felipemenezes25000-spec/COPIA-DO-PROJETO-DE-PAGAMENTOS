...
export async function login(email: string, password: string) {
  ...
  // Verificar se o usuário é admin (acesso ao painel)
...
export async function validateAdminToken(
  ...
  try {
    ...
    // Fail-closed: só aceita se o backend confirmar role de admin.
    // Não confia apenas em 200 OK — um doutor logado também tem token válido.
    ...
    // Sincroniza a role local com a retornada pelo backend (source of truth).
    localStorage.setItem(ADMIN_ROLE_KEY, role);
    return true;
  } catch {
    return false;
...
export async function approveDoctor(id: string) {
  const res = await authFetch(`/api/admin/doctors/${id}/approve`, {
    method: 'POST',
  });
