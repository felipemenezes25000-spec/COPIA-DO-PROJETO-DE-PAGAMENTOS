...
export async function getAiExamSuggestions(
  consultationId: string,
  status?: string
): Promise<AiSuggestionResponse[]> {
  const query = status ? `?status=${encodeURIComponent(status)}` : '';
  const res = await authFetch(
    `/api/consultations/${consultationId}/ai/exam-suggestions${query}`
  );
  if (!res.ok) return [];
  return res.json();
}
...
export async function createAiSuggestion(
  ...
  const res = await authFetch(
    `/api/consultations/${consultationId}/ai/exam-suggestions`,
    {
      method: 'POST',
...
export async function createCarePlanFromSuggestion(
  ...
  const res = await authFetch(
    `/api/consultations/${consultationId}/care-plans`,
    {
      method: 'POST',
      body: JSON.stringify(data),
    }
...
export async function getCarePlanByConsultation(
  consultationId: string
): Promise<CarePlan | null> {
  try {
    const res = await authFetch(
      `/api/consultations/${consultationId}/care-plan`
    );
...
export async function carePlanTaskAction(
  ...
  const res = await authFetch(
    `/api/care-plans/${carePlanId}/tasks/${taskId}/actions`,
    {
      method: 'POST',
...
export async function reviewCarePlan(
  ...
  data: {
    notes?: string;
    closePlan: boolean;
    taskDecisions: { taskId: string; decision: string; reason?: string }[];
  }
): Promise<CarePlan> {
