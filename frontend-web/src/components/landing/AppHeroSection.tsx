...
const trustIndicators = [
  {
    icon: BrainCircuit,
    text: 'IA assistiva para triagem, leitura documental e organização clínica',
  },
  {
    icon: Shield,
    text: 'Telemedicina online com rastreabilidade, LGPD e ICP-Brasil',
  },
  {
    icon: FileCheck2,
    text: 'Receitas, exames, teleconsultas e verificação pública por QR Code',
  },
];
  ...
  return (
    <section id="hero" className="relative min-h-screen overflow-hidden">
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute right-[-12rem] top-0 h-[28rem] w-[28rem] rounded-full bg-primary/20 blur-3xl" />
          ...
          >
            ...
            <div className="space-y-4">
              <h1 className="font-display text-3xl font-bold leading-tight text-foreground sm:text-5xl lg:text-6xl">
                ...
                <span className="text-primary">
                  telemedicina online com IA assistiva
                </span>{' '}
                e decisão final do médico.
              </h1>
              <p className="mx-auto max-w-2xl text-base leading-relaxed text-muted-foreground sm:text-lg lg:mx-0 lg:text-xl">
                O RenoveJá+ apoia governo, hospitais, clínicas e parceiros em
                jornadas de telemedicina online, renovação de receitas, pedidos
                de exame, teleconsultas e validação documental, com IA assistiva
                para organização do fluxo e decisão final sempre do médico, em
                linha com a Resolução CFM n.º 2.454/2026.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:justify-center lg:justify-start">
              <Button
                ...
                className="sm:h-13 h-12 gap-2 rounded-2xl px-5 text-sm font-semibold shadow-primary sm:px-7 sm:text-base"
              >
                Entrar em contato
              ...
              <Button
                ...
                className="sm:h-13 h-12 rounded-2xl border-primary/20 bg-card/70 px-5 text-sm font-semibold sm:px-7 sm:text-base"
              >
            ...
            <div className="flex flex-wrap justify-center gap-2 sm:gap-3 lg:justify-start">
              {trustIndicators.map((item) => (
                <span
                  key={item.text}
                  className="flex items-center gap-1.5 rounded-full border border-border bg-card px-3 py-1.5 text-[11px] font-medium text-foreground shadow-card sm:gap-2 sm:px-4 sm:py-2 sm:text-sm"
                >
                  <item.icon
                    className="h-3.5 w-3.5 flex-shrink-0 text-primary sm:h-4 sm:w-4"
                    aria-hidden="true"
                  />
                  {item.text}
                </span>
            ...
            <div className="relative w-full rounded-[2rem] border border-border/60 bg-card/90 p-5 shadow-2xl backdrop-blur">
              <div className="mb-5 rounded-[1.5rem] bg-gradient-to-br from-primary/15 to-primary/5 p-5">
                <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
                  ...
                  <div className="rounded-full bg-success/15 px-3 py-1 text-[10px] font-semibold text-success sm:text-xs">
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  {institutionPillars.map((pillar) => (
                    <div
                      key={pillar.title}
                      className="rounded-2xl border border-white/40 bg-white/70 p-4 shadow-sm"
                    >
                      <div
                        className="mb-3 flex h-11 w-11 items-center justify-center rounded-2xl bg-primary/10"
                        aria-hidden="true"
                      >
              ...
              <div className="space-y-3 rounded-[1.5rem] border border-border/70 bg-background p-5">
                <div className="flex flex-wrap items-center justify-between gap-2 rounded-2xl bg-muted/60 p-3 sm:p-4">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-base font-bold text-foreground sm:text-lg">
                      Telemedicina online, receita, exame e verificação
                    </p>
                  </div>
                  <div className="flex-shrink-0 rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">
                    Atual
                  </div>
                </div>
                ...
                <div className="grid gap-3">
                  {[
                    ...
                    <div
                      key={item}
                      className="flex items-center gap-3 rounded-2xl bg-muted/40 px-4 py-3"
                    >
