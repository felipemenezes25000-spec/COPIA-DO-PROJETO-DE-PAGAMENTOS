...
export function AppCTASection() {
      ...
      try {
        ...
        if (res.ok && data.ok !== false) {
          ...
          setErrorMessage(
            data.error ??
              data.message ??
              'Falha ao enviar. Tente novamente ou use o WhatsApp.'
          );
        }
      } catch {
        setStatus('error');
  ...
  return (
    <section
      id="contact"
      className="bg-app-dark relative overflow-hidden py-16 sm:py-24 lg:py-32"
    >
        ...
        <div className="grid gap-8 lg:grid-cols-[0.85fr_1.15fr] lg:items-start">
          ...
          >
            ...
            <p className="mt-6 text-lg leading-relaxed text-white/70">
              Fale com a equipe para apresentar o seu contexto, discutir
              possibilidades de uso e entender se a plataforma faz sentido para
              a sua operação.
            </p>
          ...
          <motion.form
            ...
            className="rounded-2xl border border-white/10 bg-white/5 p-4 shadow-2xl backdrop-blur-sm sm:rounded-[2rem] sm:p-8"
          >
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="block sm:col-span-2">
                ...
                <input
                  ...
                  autoComplete="name"
                  className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white placeholder:opacity-40 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/70 focus:ring-offset-2 focus:ring-offset-transparent sm:text-base"
              ...
              <label className="block">
                ...
                <input
                  value={form.cpf}
                  onChange={(e) =>
                    setForm((c) => ({ ...c, cpf: e.target.value }))
                  }
                  inputMode="numeric"
                  className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white placeholder:opacity-40 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/70 focus:ring-offset-2 focus:ring-offset-transparent sm:text-base"
              ...
              <label className="block">
                ...
                <input
                  value={form.cnpj}
                  onChange={(e) =>
                    setForm((c) => ({ ...c, cnpj: e.target.value }))
                  }
                  inputMode="numeric"
                  className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white placeholder:opacity-40 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/70 focus:ring-offset-2 focus:ring-offset-transparent sm:text-base"
            ...
            <div aria-live="polite" aria-atomic="true" className="mt-6">
              {status === 'success' && (
                <div
                  className="flex items-center gap-3 rounded-2xl border border-green-500/30 bg-green-500/10 px-4 py-3 text-green-400"
                  role="status"
                >
              ...
              {status === 'error' && (
                <div
                  className="flex items-center gap-3 rounded-2xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-red-400"
                  role="alert"
                >
