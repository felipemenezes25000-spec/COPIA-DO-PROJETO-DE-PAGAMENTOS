...
function SliderRow({
  ...
  return (
    <div className="rounded-xl border border-border/60 bg-card/80 p-3 transition-all duration-200 hover:border-primary/30">
      <label className="mb-2 flex items-center justify-between text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
        <span>{label}</span>
        <span className="font-mono text-xs text-primary">
          {fmt ? fmt(value) : NL(value)}
        </span>
      </label>
      <input
        type="range"
...
function MetricCard({
  ...
  return (
    ...
    >
      <Card className="group overflow-hidden border-border/60 bg-card/80 transition-all duration-300 hover:shadow-lg hover:shadow-primary/5">
        <CardContent className="relative p-3 text-center">
          ...
          <p className={`mt-1 font-mono text-base font-bold ${color}`}>
            {value}
          </p>
          {sub && (
            <p className="mt-0.5 text-[9px] text-muted-foreground/70">{sub}</p>
          )}
        </CardContent>
      </Card>
    </motion.div>
...
/* ─── Main component ─── */
export function FunilConsultaTab({
  pacientesMes,
  durMedia,
  diasMes,
}: FunilConsultaTabProps) {
  ...
  useEffect(() => {
    setSimPacientes(pacientesMes);
  }, [pacientesMes]);
  const [pctSimples, setPctSimples] = useState(
    Math.round(
      (CONSULTATION_OUTCOMES.find((o) => o.serviceId === 'receita_simples')
        ?.probability ?? 0.55) * 100
    )
  );
  const [pctControlada, setPctControlada] = useState(
    Math.round(
      (CONSULTATION_OUTCOMES.find((o) => o.serviceId === 'receita_controlada')
        ?.probability ?? 0.12) * 100
    )
  );
  const [pctExameLab, setPctExameLab] = useState(
    Math.round(
      (CONSULTATION_OUTCOMES.find((o) => o.serviceId === 'exame_lab')
        ?.probability ?? 0.35) * 100
    )
  );
  const [pctExameImg, setPctExameImg] = useState(
    Math.round(
      (CONSULTATION_OUTCOMES.find((o) => o.serviceId === 'exame_imagem')
  ...
  const funnelData = useMemo(() => {
    ...
    const outcomes = CONSULTATION_OUTCOMES.filter(
      (o) => RENOVEJA_SERVICES[o.serviceId].price > 0
    );
    ...
    const totalPerConsulta = calcTotalRevenuePerConsultation(
      durMedia,
      'clinica'
    );
  ...
  const funnelChartData = useMemo(() => {
    const labels = [
      'Teleconsulta',
      ...funnelData.derivs.map((d) => d.name.split(' ').slice(0, 2).join(' ')),
    ];
    const values = [
      funnelData.consultaRev,
      ...funnelData.derivs.map((d) => d.expected),
    ];
    const colors = [
      SERVICE_COLORS.consulta_clinica,
      ...funnelData.derivs.map((d) => d.color),
    ];
    return {
      labels,
      datasets: [
        ...
        tooltip: {
          callbacks: {
            label: (ctx: TooltipItem<'bar'>) =>
              ` R$ ${F2((ctx.parsed as { x: number }).x ?? 0)}`,
          },
        },
      },
      scales: {
        x: {
          ...
          ticks: {
            color: CHART_COLOR_TEXT,
            font: CHART_FONT,
            callback: (v: number | string) =>
              `R$${+v < 1 ? F2(+v) : Math.round(+v)}`,
          },
          title: {
            display: true,
            text: 'Receita esperada por teleconsulta (R$)',
            color: CHART_COLOR_TEXT,
            font: CHART_FONT,
          },
        },
  ...
  const profileRevenues = useMemo(
    () =>
      PATIENT_PROFILES.map((p) => {
        const consultaRev = calcConsultationRevenue(
          p.avgConsultDurationMin,
          'clinica'
        );
        ...
        return {
          ...
          yearlyRevPerPatient:
            Math.round((consultaRev + derivTotal) * p.consultasPerYear * 100) /
            100,
        };
      }),
    []
  ...
  const weightedAvgDur = useMemo(() => {
    ...
    return profileRevenues.reduce(
      (s, pr, i) => s + pr.avgConsultDurationMin * (mixRaw[i] / mixSum),
      0
    );
  ...
  const desdobSvc = useMemo(() => {
    const items: Array<{
      ...
      },
      {
        ...
        avgQty:
          CONSULTATION_OUTCOMES.find(
            (o) => o.serviceId === 'receita_controlada'
          )?.avgQuantity ?? 1.1,
  ...
  const desdobCalc = useMemo(() => {
    const consultaRevMes =
      calcConsultationRevenue(durMedia, 'clinica') * simPacientes;
    const consultaCostMes =
      RENOVEJA_SERVICES.consulta_clinica.costToDeliver *
      durMedia *
      simPacientes;
    ...
    const totalRevMes =
      Math.round((consultaRevMes + derivRevTotal) * 100) / 100;
    const totalCostMes =
      Math.round((consultaCostMes + derivCostTotal) * 100) / 100;
    const margemBruta = Math.round((totalRevMes - totalCostMes) * 100) / 100;
    const revPorPaciente =
      simPacientes > 0
        ? Math.round((totalRevMes / simPacientes) * 100) / 100
        : 0;
  ...
  const desdobBarData = useMemo(
    () => ({
      labels: [
        'Teleconsulta',
        ...desdobCalc.rows.map((r) => r.name.split(' ').slice(0, 2).join(' ')),
      ],
      datasets: [
        {
          label: 'Receita (R$)',
          data: [
            desdobCalc.consultaRevMes,
            ...desdobCalc.rows.map((r) => r.revenue),
          ],
          backgroundColor: [
            SERVICE_COLORS.consulta_clinica,
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const consultaFrac =
        desdobCalc.totalRevMes > 0
          ? desdobCalc.consultaRevMes / desdobCalc.totalRevMes
          : 0.5;
      months.push(`M${i + 1}`);
    ...
    <div className="space-y-8 pb-8">
          ...
          <Card className="border-border/60 bg-card/80 lg:col-span-2">
            <CardContent className="p-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex items-center justify-between text-xs">
                  <span className="flex items-center gap-2">
                    <span
                      ...
                      style={{ background: SERVICE_COLORS.consulta_clinica }}
                    />
                ...
                <div className="mt-2 flex items-center justify-between border-t border-border/60 pt-2 text-xs font-bold">
                  <span className="text-foreground">Total esperado</span>
                  <span className="font-mono text-yellow-400">
                    R$ {F2(funnelData.totalPerConsulta)}
                  </span>
                </div>
              </div>
            </CardContent>
      ...
      >
        ...
        {Math.abs(mixSum - 100) > 1 && (
          <div className="mb-3 rounded-lg border border-yellow-400/20 bg-yellow-400/10 px-3 py-2 text-[10px] font-semibold text-yellow-400">
            Soma do mix: {mixSum}% — os percentuais não somam 100%. A ponderação
            usará a proporção relativa.
          </div>
        )}
        ...
        <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-5">
          {PATIENT_PROFILES.map((profile, idx) => {
            ...
            return (
              ...
              >
                <Card className="h-full border-border/60 bg-card/80 transition-all duration-200 hover:border-primary/30">
                  <CardContent className="p-3">
                    <div className="mb-2 flex items-start justify-between">
                      <div className="min-w-0">
                        ...
                        <p className="mt-0.5 line-clamp-2 text-[9px] leading-tight text-muted-foreground/80">
                          {profile.description}
                        </p>
                      </div>
                      <span className="ml-1 flex-shrink-0 font-mono text-[9px] font-bold text-primary">
                        {mixRaw[idx]}%
                      </span>
                    </div>
                    ...
                    <div className="mb-2 space-y-1">
                      ...
                      <div className="flex justify-between text-[9px]">
                        ...
                        <span className="font-mono">
                          {profile.avgConsultDurationMin} min
                        </span>
                      </div>
                      <div className="flex justify-between text-[9px]">
                        <span className="text-muted-foreground">
                          Receita total/visita (teleconsulta + derivados)
                        </span>
                        <span className="font-mono text-green-400">
                          R$ {F2(pr.totalPerVisit)}
                        </span>
                      </div>
                      <div className="flex justify-between text-[9px]">
                        <span className="text-muted-foreground">
                    ...
                    <div className="mb-2 h-1 w-full rounded-full bg-secondary/60">
                      <div
                    ...
                    <input
                      type="range"
                      className="h-1 w-full rounded-full accent-primary"
                      min={0}
                      max={100}
                      step={1}
                      value={mixRaw[idx]}
      ...
      >
          ...
          <Card className="border-border/60 bg-card/80">
            <CardContent className="p-4">
              ...
              <div className="space-y-2">
                {/* Column headers */}
                <div className="flex items-center justify-between gap-2 border-b border-border/40 pb-1 text-[9px] font-semibold uppercase tracking-wider text-muted-foreground">
                  <span>Serviço</span>
                ...
                <div className="flex items-center justify-between gap-2 text-xs">
                  ...
                  <span className="flex flex-shrink-0 items-center gap-3">
                    ...
                    <span className="w-20 text-right font-mono text-primary">
                      {FK(desdobCalc.consultaRevMes)}
                    </span>
                  </span>
                </div>
                ...
                {desdobCalc.rows.map((row) => (
                  ...
                  >
                    <span className="flex min-w-0 items-center gap-2">
                      <span
                        className="h-2 w-2 flex-shrink-0 rounded-sm"
                        style={{ background: SERVICE_COLORS[row.id] }}
                      />
                      <span className="truncate text-muted-foreground">
                    ...
                    <span className="flex flex-shrink-0 items-center gap-3">
                      ...
                      <span className="w-20 text-right font-mono text-green-400">
                        {FK(row.revenue)}
                      </span>
                    </span>
                  </div>
              ...
              <div className="mt-3 space-y-1.5 border-t border-border/60 pt-3">
                <div className="flex justify-between text-xs font-bold">
                  ...
                  <span className="font-mono text-yellow-400">
                    {FK(desdobCalc.totalRevMes)}
                  </span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-muted-foreground">Custo total</span>
                  <span className="font-mono text-red-400">
                    {FK(desdobCalc.totalCostMes)}
                  </span>
                </div>
                <div className="flex justify-between text-xs font-bold">
                  <span>Margem bruta</span>
                  <span
                    className={`font-mono ${desdobCalc.margemBruta >= 0 ? 'text-green-400' : 'text-red-400'}`}
                  >
                ...
                <div className="flex justify-between text-xs">
                  ...
                  <span className="font-mono text-primary">
                    R$ {F2(desdobCalc.revPorPaciente)}
                  </span>
                </div>
                {desdobCalc.totalRevMes > 0 && (
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-mono text-muted-foreground">
                      {F2(
                        (desdobCalc.margemBruta / desdobCalc.totalRevMes) * 100
                      )}
                      %
                    </span>
                  </div>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <MetricCard
            ...
            sub={
              desdobCalc.totalRevMes > 0
                ? `${F2((desdobCalc.margemBruta / desdobCalc.totalRevMes) * 100)}% da receita`
                : '—'
            }
            color={
              desdobCalc.margemBruta >= 0 ? 'text-green-400' : 'text-red-400'
            }
            delay={0.08}
          />
          <MetricCard
