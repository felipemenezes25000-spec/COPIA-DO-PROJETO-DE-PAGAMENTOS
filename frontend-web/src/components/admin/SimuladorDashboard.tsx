...
function KpiCard({
  ...
  return (
    ...
    >
      <Card className={`border ${statusBorder(status)} h-full bg-card`}>
        <CardContent className="p-4">
          <div className="mb-2 flex items-start justify-between gap-2">
            ...
            <span
              className={`mt-0.5 h-2 w-2 flex-shrink-0 rounded-full ${statusBg(status)}`}
            />
          </div>
          <p
            className={`font-mono text-lg font-bold leading-tight ${statusColor(status)}`}
          >
            {value}
          </p>
...
export function DashboardTab({
  ...
  const calcs = useMemo(() => {
    ...
    const derivClinica = calcDerivativeRevenue(
      CONSULTATION_OUTCOMES,
      pacientesClinica
    );
    ...
    const utilizacaoPct =
      capacidadeTotal > 0 ? (pacientesMes / capacidadeTotal) * 100 : 0;

    // LTV & CAC
    const avgConsultasYear =
      PATIENT_PROFILES.reduce(
        (s, p) => s + p.consultasPerYear * p.populationPct,
        0
    ...
    const roiAnual =
      custoTotal > 0 ? ((resultadoLiquido * 12) / (custoTotal * 12)) * 100 : 0;
    const breakeven = resultadoLiquido > 0 ? custoTotal / resultadoLiquido : 99; // months approx.
  ...
  const serviceComposition = useMemo(() => {
    ...
    return rows
      .map((r) => ({
        ...r,
        pct: total > 0 ? (r.monthlyRevenue / total) * 100 : 0,
      }))
  ...
  const doughnutData = useMemo(
    () => ({
      labels: serviceComposition.map((r) =>
  ...
  const profileMetrics = useMemo(() => {
    return PATIENT_PROFILES.map((profile) => {
      ...
      const annualRevenuePerPatient =
        revenuePerVisit * profile.consultasPerYear;
      const patientsInSegment = Math.round(
        pacientesMes * profile.populationPct
      );
      const monthlyContribution =
        patientsInSegment * (annualRevenuePerPatient / 12);
  ...
  const healthScore = useMemo(() => {
    const { margemBruta, ltvCac, utilizacaoPct, resultadoLiquido, breakeven } =
      calcs;
    const topServicePct =
      serviceComposition.length > 0 ? serviceComposition[0].pct : 100;
    ...
    if (calcs.margemBruta <= 30)
      recommendations.push(
        'Aumente o ticket médio ampliando serviços derivados por teleconsulta.'
      );
    if (ltvCac <= 3)
      recommendations.push(
        'Reduza o CAC com indicações ou aumente retenção com assinatura mensal.'
      );
    if (utilizacaoPct <= 70)
      recommendations.push(
        'Otimize escala de médicos ou amplie o marketing digital para aumentar a taxa de utilização.'
      );
    if (topServicePct >= 60)
      recommendations.push(
        'Diversifique o mix de receita estimulando receituários e exames.'
      );
    if (calcs.breakeven >= 6)
      recommendations.push(
        'Reduza custos fixos de pessoal nos primeiros meses via modelo PJ/flexível.'
      );
    if (recommendations.length === 0)
      recommendations.push(
        'Operação saudável. Considere expansão de cobertura geográfica.'
      );
  ...
  const profileIcons: Record<string, React.ReactNode> = {
    jovem_saudavel: <Heart className="h-4 w-4" />,
    adulto_cronico: <Pill className="h-4 w-4" />,
    idoso_polimedicado: <Stethoscope className="h-4 w-4" />,
    saude_mental: <Brain className="h-4 w-4" />,
    eventual: <Clock className="h-4 w-4" />,
  };
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
          ...
          <KpiCard
            ...
            value={calcs.breakeven >= 99 ? '∞' : `${fNL(calcs.breakeven, 1)} m`}
                ...
                <thead>
                  <tr className="border-b border-border">
                    {[
                      ...
                      <th
                        key={h}
                        className="px-4 py-2.5 text-left font-semibold uppercase tracking-wide text-muted-foreground"
                      >
                        {h}
                      </th>
                    ))}
                  ...
                  {serviceComposition.map((row, i) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-2 font-mono text-muted-foreground">
                        {row.id === 'consulta_clinica' ||
                        row.id === 'consulta_psico'
                          ? `${fK(row.price)}/min`
                          : fK(row.price)}
                      </td>
                      <td className="px-4 py-2 font-mono text-muted-foreground">
                      ...
                      <td className="px-4 py-2">
                        <div className="flex items-center gap-1.5">
                          <div className="h-1.5 max-w-16 flex-1 rounded-full bg-indigo-500/30">
                            <div
      ...
      <section>
        ...
        >
          <Card>
            <CardContent className="p-5">
              <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                Sensibilidade de Margem — Variação de Pacientes/Mês (50% → 200%
                do cenário atual)
              </p>
              <div className="h-44">
                <Line
                ...
                <CardContent className="p-4">
                  <div className="mb-3 flex items-center gap-2">
                    <span className="text-indigo-400">
                      {profileIcons[p.id] ?? <Users className="h-4 w-4" />}
                    </span>
                    <span className="text-sm font-semibold leading-tight text-foreground">
                      {p.name}
                    </span>
                  </div>
                  <p className="mb-4 line-clamp-3 text-xs leading-snug text-muted-foreground">
                    {p.description}
                  </p>
                  <dl className="space-y-1.5">
                    {(
                      ...
                      >
                        ...
                        <dd className="whitespace-nowrap font-mono text-xs font-semibold text-foreground">
                          {v}
                        </dd>
                      </div>
                    ))}
            ...
            <Card className="h-full">
              <CardContent className="p-5">
                ...
                <div className="mt-5 border-t border-border pt-4">
                  ...
                  <div className="mt-1 flex justify-between border-t border-border py-1 text-xs font-semibold">
                ...
                <div className="mt-4 flex flex-wrap gap-2 border-t border-border pt-3">
                  {[
                    ...
                    { icon: <Brain className="h-3 w-3" />, label: 'Psico' },
                    { icon: <Pill className="h-3 w-3" />, label: 'Receita' },
                    {
                      icon: <FlaskConical className="h-3 w-3" />,
                      label: 'Lab',
                    },
                    { icon: <Scan className="h-3 w-3" />, label: 'Imagem' },
                  ].map(({ icon, label }) => (
                    <span
                      key={label}
                      className="flex items-center gap-1 rounded bg-secondary/40 px-1.5 py-0.5 text-xs text-muted-foreground"
                    >
                      {icon}
                      {label}
                    </span>
