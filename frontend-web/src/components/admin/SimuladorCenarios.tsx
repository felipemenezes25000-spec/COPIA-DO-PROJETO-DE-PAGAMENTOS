...
function calcScenario(
  ...
  const derivPerClinica = Math.max(
    0,
    revPerClinica - p.avgConsultDuration * 6.99
  );
  ...
  const roiAnual = costTotalMonth > 0 ? (profitMonth * 12) / costTotalMonth : 0;
  ...
  return (
    <div className="rounded-xl border border-border/60 bg-card/80 p-3 transition-all duration-200 hover:border-primary/30">
      ...
      <div className="flex items-center gap-2">
        <input
          type="range"
          className="h-1.5 flex-1 rounded-full accent-primary"
          min={min}
          max={max}
          step={step}
          value={value}
        ...
        />
        <span className="w-14 text-right font-mono text-sm text-primary">
...
function OutcomeSlider({
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <input
        type="range"
        className="h-1 flex-1 rounded-full accent-primary"
        min={0}
        max={100}
        step={1}
...
export function CenariosTab({ valConsulta, diasMes }: CenariosTabProps) {
  ...
  const customOutcomes = useMemo(
    () =>
      CONSULTATION_OUTCOMES.map((o) => {
        ...
        if (o.serviceId === 'exame_lab')
          return { ...o, probability: pExameLab };
  ...
  const mixStats = useMemo(() => {
    ...
    const consultsPerDoctorPerMonth =
      Math.floor((baseParams.hoursPerDay * 60) / avgDuration) * effectiveDias;
    ...
    const staffCost =
      baseParams.doctorsPerShift * baseParams.doctorCostPerDay * effectiveDias +
      ...
      datasets: [
        ...
        },
        {
          ...
          borderColor: allFinancials.map((f) =>
            f.fin.profitMonth >= 0 ? 'rgba(34,197,94,1)' : 'rgba(239,68,68,1)'
          ),
          borderWidth: 1,
          borderRadius: 4,
        },
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          {allFinancials.map(({ sc, fin }, i) => {
            ...
            return (
              ...
              >
                <Card
                  className={`cursor-pointer overflow-hidden border transition-all duration-200 ${
                    isSelected
                  ...
                  <CardContent className="space-y-3 p-4">
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          {isBest && (
                            <span className="rounded-full border border-green-500/30 bg-green-500/20 px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-wider text-green-400">
                              Mais lucrativo
                            </span>
                          )}
                        </div>
                    ...
                    <div className="border-t border-border/40 pt-2">
                      <div className="flex items-center justify-between">
                        ...
                        <span
                          className={`font-mono text-xs font-bold ${profitColor(fin.profitMonth)}`}
                        >
                          {FK(fin.profitMonth)}
                        </span>
                      </div>
                      <div className="mt-0.5 flex items-center justify-between">
                        ...
                        <span
                          className={`font-mono text-xs font-bold ${marginColor(fin.marginPct)}`}
                        >
                          {F2(fin.marginPct)}
                        </span>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
                        {[
                          ...
                          >
                            ...
                            <p
                              className={`mt-1 font-mono text-base font-bold ${color}`}
                            >
                              {value}
                            </p>
                          </div>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-border/60 bg-secondary/30">
                    <th className="w-44 px-4 py-3 text-left text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
                      Métrica
                    </th>
                    {allFinancials.map(({ sc }) => (
                      <th
                        key={sc.id}
                        className={`px-3 py-3 text-center text-[10px] font-bold uppercase tracking-wide ${
                          sc.id === bestId
                            ? 'text-green-400'
                            : 'text-foreground'
                        }`}
                      >
                        ...
                        {sc.name}
                        {sc.id === bestId && (
                          <span className="ml-1 rounded-full border border-green-500/30 bg-green-500/20 px-1 py-0.5 text-[7px] text-green-400">
                            ★
                          </span>
                        )}
                      </th>
                ...
                <tbody className="divide-y divide-border/30">
                  {(
                    ...
                    <tr
                      key={row.label}
                      className={
                        ri % 2 === 0 ? 'bg-transparent' : 'bg-secondary/10'
                      }
                    >
                      <td className="whitespace-nowrap px-4 py-2.5 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">
                        {row.label}
                      </td>
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          <Card className="border-border/60 bg-card/80">
            <CardContent className="space-y-4 p-5">
              ...
              <p className="flex items-center gap-2 pt-2 text-xs font-bold uppercase tracking-wider text-primary">
                <span className="h-4 w-1 rounded-full bg-primary" />
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          <Card className="border-border/60 bg-card/80">
            <CardContent className="p-5">
              ...
              {/* Phase legend */}
              <div className="mb-1 flex flex-wrap gap-3 text-[9px] text-muted-foreground">
                <span className="self-center text-[8px] font-bold uppercase tracking-wider text-muted-foreground/50">
                  Fases:
                </span>
                {[
