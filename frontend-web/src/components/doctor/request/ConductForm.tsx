...
function parseStructuredFromLegacy(text: string): ConductFormData {
  ...
  for (const line of lines) {
    ...
    if (
      lower.startsWith('hipóte') ||
      lower.startsWith('hipote') ||
...
function combineToConductText(data: ConductFormData): string {
  const parts: string[] = [];
  if (data.queixaDuracao.trim())
    parts.push(`Queixa e duração: ${data.queixaDuracao.trim()}`);
  if (data.evolucao.trim())
    parts.push(`Evolução / Anamnese: ${data.evolucao.trim()}`);
  if (data.hipoteseCid.trim())
    parts.push(`Hipótese diagnóstica (CID): ${data.hipoteseCid.trim()}`);
...
const FIELDS: {
  ...
  },
  {
    ...
    placeholder:
      'Ex.: Paciente refere piora progressiva, sem irradiação, uso de analgésico sem melhora...',
    rows: 3,
    icon: Clock,
  },
  {
...
export function ConductForm({
  ...
  useEffect(() => {
    ...
    if (anamnesis) {
      setForm((prev) => {
        const next = { ...prev };
        if (
          !next.queixaDuracao.trim() &&
          typeof anamnesis.queixa_principal === 'string'
        ) {
          next.queixaDuracao = anamnesis.queixa_principal;
        }
        if (
          !next.evolucao.trim() &&
          typeof anamnesis.historia_doenca_atual === 'string'
        ) {
        ...
        if (!next.conduta.trim()) {
          ...
          if (Array.isArray(meds) && meds.length > 0) {
            condutaParts.push(
              'Medicamentos: ' +
                meds.map((m) => displayMedicamento(m)).join('; ')
            );
          }
          const exams = anamnesis.exames_sugeridos;
          if (Array.isArray(exams) && exams.length > 0) {
            condutaParts.push(
              'Exames: ' + exams.map((e) => displayExame(e)).join('; ')
            );
          }
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      await updateConduct(requestId, {
        conductNotes: combined,
        includeConductInPdf: includeInPdf,
      });
  ...
  const prefillFromAnamnesis = useCallback(() => {
    const anamnesis = parseAnamnesis(anamnesisJson);
    if (!anamnesis) {
      toast.info('Anamnese não disponível');
      return;
    }
        ...
        {aiSuggestion?.trim() && (
          <div className="overflow-hidden rounded-lg border border-primary/20 bg-primary/[0.03]">
            <button
              ...
              className="flex w-full items-center justify-between p-3 text-left transition-colors hover:bg-primary/[0.05]"
            >
            ...
            {aiExpanded && (
              <div className="space-y-2 border-t border-primary/10 px-3 pb-3">
                <p className="whitespace-pre-wrap pt-2 text-sm text-muted-foreground">
                  {aiSuggestion.length > 500
                    ? aiSuggestion.slice(0, 500) + '...'
                    : aiSuggestion}
                </p>
                <Button
                  variant="outline"
