...
export function TemplateDialogSave({
  ...
  const handleSave = () => {
    ...
    if (mode === 'prescription' && prescriptionData) {
      ...
      // Deep copy: evita que edições posteriores no editor atual alterem o template salvo
      // (as referências de medications eram compartilhadas, corrompendo o localStorage).
      templates.unshift({
        name: trimmed,
        medications: prescriptionData.medications.map((m) => ({ ...m })),
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Dê um nome ao template para reutilizar depois (ex: &quot;Amoxicilina
            padrão&quot;).
          </DialogDescription>
        </DialogHeader>
...
export function TemplateDialogLoad({
  ...
  const handleDelete = (
    t: PrescriptionTemplate | ExamTemplate,
    e: React.MouseEvent
  ) => {
    e.stopPropagation();
    if (deleteConfirm === t.name) {
      if (mode === 'prescription') {
        const list = loadPrescriptionTemplates().filter(
          (x) => x.name !== t.name
        );
