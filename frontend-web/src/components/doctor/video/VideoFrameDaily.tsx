...
export function VideoFrameDaily({
  ...
  useEffect(() => {
    onCallLeftRef.current = onCallLeft;
  }, [onCallLeft]);
    ...
    function handleVisibilityChange() {
      ...
      try {
        if (document.visibilityState === 'hidden') {
          ...
          frameRef.current.setBandwidth({
            kbs: 50,
            trackConstraints: { width: 320, height: 240, frameRate: 5 },
          });
        } else {
          // Restore full bandwidth when tab visible again
          frameRef.current.setBandwidth({
            kbs: 'NO_CAP',
            trackConstraints: { width: 1280, height: 720, frameRate: 30 },
          });
        }
      } catch {
    ...
    void runDailyWebExclusive(async () => {
      ...
      timeoutRef.current = setTimeout(() => {
        if (!destroyedRef.current && frameRef.current) {
          ...
          toast.error(
            'Tempo limite de conexão atingido. A videochamada pode estar indisponível.'
          );
        }
      ...
      frame.on('network-connection', (event) => {
        ...
        if (ev.type === 'disconnected' || ev.action === 'interrupted') {
          ...
          toast.warning('Conexão interrompida. Tentando reconectar...', {
            id: 'daily-reconnecting',
          });
      ...
      frame.on('nonfatal-error', (event) => {
        ...
        if (ev.type === 'mic-error') {
          ...
          if (ev.errorMsg?.includes('not-found')) {
            ...
            ev.errorMsg?.includes('not-allowed') ||
            ev.errorMsg?.includes('permissions')
          ) {
            message =
      ...
      frame.on('track-stopped', (event) => {
        ...
        const ev = event as {
          track?: { kind?: string };
          participant?: { local?: boolean; screen?: boolean };
        };
  ...
  return (
    <div
      className={`relative bg-[#0B1120] transition-all duration-300 ${
        isExpanded ? 'w-[40%]' : 'w-[60%]'
      } max-md:h-[50vh] max-md:min-h-[220px] max-md:!w-full landscape:max-md:h-[60vh]`}
    >
      <div className="relative h-full min-h-[200px] w-full">
        <div
          ref={containerRef}
          className="h-full min-h-[200px] w-full bg-[#0B1120]"
        />
        ...
        {meetingJoined && !reconnecting && !connectionTimedOut && (
          <div className="absolute left-3 top-3 z-10 flex items-center gap-1.5 rounded-full bg-black/50 px-2.5 py-1 text-[10px] font-medium text-gray-300 backdrop-blur-sm sm:left-4 sm:top-4 sm:text-xs">
        ...
        {reconnecting && (
          <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-[#0B1120]/90 backdrop-blur-sm">
            <WifiOff className="mb-3 h-8 w-8 animate-pulse text-amber-400 sm:h-10 sm:w-10" />
            <p className="text-sm font-medium text-amber-300">
              Reconectando...
            </p>
            <p className="mt-1 text-xs text-gray-500">
              Aguarde, tentando restabelecer a conexão
            </p>
            <Loader2 className="mt-3 h-5 w-5 animate-spin text-amber-400" />
          </div>
        )}
        ...
        {connectionTimedOut && (
          <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-[#0B1120]/95 backdrop-blur-sm">
            <AlertTriangle className="mb-3 h-8 w-8 text-red-400 sm:h-10 sm:w-10" />
            <p className="text-sm font-medium text-red-300">
              Tempo de conexão esgotado
            </p>
            <p className="mt-1 max-w-xs px-4 text-center text-xs text-gray-500">
              Não foi possível conectar à videochamada em{' '}
              {CONNECTION_TIMEOUT_MS / 1000}s.
            </p>
            <Button
              onClick={handleRetry}
              variant="outline"
        ...
        {screenShareActive && (
          <div className="absolute left-3 top-3 z-10 flex items-center gap-2 rounded-full bg-emerald-900/80 px-3 py-1.5 text-xs font-medium text-emerald-300 backdrop-blur-sm sm:left-4 sm:top-4">
        ...
        <button
          ...
          className="absolute right-3 top-3 z-10 rounded-lg bg-black/50 p-1.5 text-gray-400 backdrop-blur-sm transition-colors hover:text-white sm:right-4 sm:top-4 sm:p-2"
