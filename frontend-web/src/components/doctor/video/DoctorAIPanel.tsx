...
export function DoctorAIPanel({
  ...
  const denominadorComum =
    (anamnesis?.denominador_comum as string)?.trim() || undefined;
  ...
  const exameFisicoDirigido =
    (anamnesis?.exame_fisico_dirigido as string) ?? '';
  const orientacoesPaciente: string[] = useMemo(() => {
    try {
      return Array.isArray(anamnesis?.orientacoes_paciente)
        ? (anamnesis!.orientacoes_paciente as string[])
        : [];
    } catch {
      return [];
    }
  ...
  const criteriosRetorno: string[] = useMemo(() => {
    try {
      return Array.isArray(anamnesis?.criterios_retorno)
        ? (anamnesis!.criterios_retorno as string[])
        : [];
    } catch {
      return [];
    }
  ...
  const perguntasSugeridas: PerguntaSugerida[] = useMemo(() => {
    try {
      ...
      return (anamnesis!.perguntas_sugeridas as unknown[])
        .map((p: unknown) =>
          typeof p === 'string'
            ? {
                pergunta: p,
            ...
            : {
                ...
                hipoteses_afetadas:
                  (p as PerguntaSugerida).hipoteses_afetadas ?? '',
                impacto_na_conduta:
                  (p as PerguntaSugerida).impacto_na_conduta ?? '',
                prioridade: ((p as PerguntaSugerida).prioridade ?? 'media') as
                  | 'alta'
                  | 'media'
                  | 'baixa',
              }
  ...
  const lacunasAnamnese: string[] = useMemo(() => {
    try {
      return Array.isArray(anamnesis?.lacunas_anamnese)
        ? (anamnesis!.lacunas_anamnese as string[])
        : [];
    } catch {
      return [];
    }
  ...
  const alertasVermelhos: string[] = useMemo(() => {
    try {
      return Array.isArray(anamnesis?.alertas_vermelhos)
        ? (anamnesis!.alertas_vermelhos as string[])
        : [];
    } catch {
      return [];
    }
  ...
  const meds: MedSugerido[] = useMemo(() => {
    try {
      return Array.isArray(anamnesis?.medicamentos_sugeridos)
        ? (anamnesis!.medicamentos_sugeridos as MedSugerido[])
        : [];
    } catch {
      return [];
    }
  ...
  const exames: ExameSugerido[] = useMemo(() => {
    try {
      return Array.isArray(anamnesis?.exames_sugeridos)
        ? (anamnesis!.exames_sugeridos as ExameSugerido[])
        : [];
    } catch {
      return [];
    }
  ...
  const primaryHipotese =
    diagDiferencial.length > 0 ? (diagDiferencial[0].hipotese ?? '') : '';
  const primaryCid =
    diagDiferencial.length > 0 ? (diagDiferencial[0].cid ?? '') : '';
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-[#15202E]">
      {/* Tab bar - horizontally scrollable on small screens to prevent overflow */}
      <div className="scrollbar-none flex shrink-0 gap-1 overflow-x-auto border-b border-white/5 p-2">
        {TABS.map((t) => (
          <button
            key={t.key}
            type="button"
            onClick={() => setActiveTab(t.key)}
            className={`shrink-0 whitespace-nowrap rounded-md px-2.5 py-1.5 text-[11px] font-medium transition-colors sm:px-3 sm:text-xs ${
              activeTab === t.key
      ...
      {/* Content area */}
      <div className="flex-1 space-y-3 overflow-auto p-3 sm:space-y-4 sm:p-4">
        {activeTab === 'consulta' && (
          <>
            <AIIndicators
              gravidade={gravidade}
                    ...
                    <div
                      ...
                      className={`flex gap-2 rounded-lg p-2.5 text-sm ${
                        isDanger
                          ...
                          : 'border border-[#8B5CF6]/20 bg-[#8B5CF6]/10'
                      }`}
                    >
                      {isDanger ? (
                        <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-red-400" />
                      ) : (
                        <Lightbulb className="mt-0.5 h-4 w-4 shrink-0 text-[#8B5CF6]" />
                      )}
                      <span
                        className={`text-xs sm:text-sm ${isDanger ? 'text-red-300' : 'text-purple-200'}`}
                      >
        ...
        {activeTab === 'evidencias' && <AIEvidencePanel evidence={evidence} />}
      </div>
    </div>
  );
}
