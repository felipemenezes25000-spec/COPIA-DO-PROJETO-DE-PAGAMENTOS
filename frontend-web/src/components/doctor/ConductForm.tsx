...
function parseAnamnesisForPrefill(
...
export function ConductForm({
  ...
  const applyAiSuggestion = useCallback(() => {
    ...
    if (anamnesis?.cid_sugerido && !form.hipoteseCid.trim()) {
      ...
      const desc = anamnesis.cid_descricao
        ? ` — ${anamnesis.cid_descricao}`
        : '';
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`flex h-5 w-5 shrink-0 items-center justify-center rounded border-2 transition-colors focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 ${
              includeConductInPdf
