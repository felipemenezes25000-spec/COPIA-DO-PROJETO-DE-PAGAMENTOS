...
describe('resetPassword', () => {
  ...
  it('deve lançar erro com mensagem do servidor quando resposta não é ok', async () => {
    ...
    await expect(resetPassword('token-invalido', 'Senha@123')).rejects.toThrow(
      'Token expirado'
    );
  });
  ...
  it('deve usar campo message quando error não existe na resposta', async () => {
    ...
    await expect(resetPassword('token', 'abc')).rejects.toThrow(
      'Senha muito fraca'
    );
  });
  ...
  it('deve usar mensagem genérica quando servidor retorna JSON inválido', async () => {
    ...
    await expect(resetPassword('token', 'Senha@123')).rejects.toThrow(
      'Erro ao redefinir senha (500)'
    );
  });
  ...
  it('deve resolver sem erro quando resposta é ok', async () => {
    ...
    await expect(
      resetPassword('token-valido', 'NovaSenha@123')
